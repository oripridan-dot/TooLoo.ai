...
hotReloadManager.watchFile('src/servers/web-server.ts', async () => {
...
const serviceCircuitBreakers = {
  ...
  guide: new CircuitBreaker('guide-service', { failureThreshold: 3, resetTimeoutMs: 30000 }),
};
...
app.get(
  ['/chat', '/chat-pro', '/chat-pro-v2', '/chat-v2', '/chat-enhanced', '/professional'],
  (req, res) => {
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
    res.set('Pragma', 'no-cache');
    res.set('Expires', '0');
  ...
  try {
    ...
    try {
      ...
      if (isEvolveRequest) {
        ...
        let evolutionResult = { applied: false, message: 'Evolution engine not ready' };
...
Now synthesize these perspectives into a single, unified answer that directly addresses the user's question. Be confident and specific.`;
    ...
    return providerResponses.map((r, idx) => `${r.provider}: ${r.response}`).join('\n\n');
  } catch (error) {
        ...
        const awareness = {
          system: {
            ...
            serviceDetails: {
              ...
              acquisition: 'Meta-learning phases, learning optimization, boost strategies',
          ...
          integrations: {
            figma: {
              enabled: true,
              capability: 'Full Figma API integration for design token import and CSS generation',
              endpoints: [
...
- Be specific and confident about TooLoo.ai's features, integrations, and services`;
      ...
      const aggregationResult = await aggregation.callAllProviders(enrichedMessage, {
        taskType: 'chat',
      ...
      const providerCalls = providers.map((provider) =>
        (async () => {
          try {
            ...
            const systemPrompt = instructions.buildSpecializedPrompt(provider, '', {
              taskType: 'validation',
              aggregationContext: true,
            });
...
app.post('/api/v1/chat/smart-intelligence', async (req, res) => {
  try {
    const { question, responseText, providerResponses, metadata = {} } = req.body || {};
    ...
    if (providerResponses && Array.isArray(providerResponses)) {
      const validator = new ResponseCrossValidator();
      crossValidationResult = validator.validateResponses(question, providerResponses);
    }
    ...
    if (crossValidationResult) {
      ...
      analysisResult = analyzer.analyzeValidationReport(responseText, crossValidationResult);
    } else {
      // Single response - use single-response analysis
      analysisResult = analyzer.analyzeSingleResponse(responseText, question, metadata);
    }
...
app.get('/temp/index', async (req, res) => {
  try {
    ...
    const chats = files
      ...
      .reverse();
    const li = (arr) =>
      arr.map((f) => `<li><a href="/temp/${f}" target="_blank">${f}</a></li>`).join('');
...
app.post('/api/v1/referral/redeem', async (req, res) => {
  try {
    ...
    if (!code || !newUserId)
      return res.status(400).json({ ok: false, error: 'code and newUserId required' });
...
app.get('/api/v1/knowledge/status', async (req, res) => {
  try {
    if (!tier1KnowledgeEnhancementEngine) {
      return res
        .status(503)
        .json({ ok: false, error: 'Knowledge enhancement engine not initialized' });
    }
...
app.get('/api/v1/knowledge/sources', async (req, res) => {
  try {
    ...
    const sources = await tier1KnowledgeEnhancementEngine.webSourceEngine.getSourcesForTopic(
      topic,
      {
        limit,
        minAuthority,
...
app.get('/api/v1/knowledge/memory/patterns', async (req, res) => {
  try {
    ...
    const patterns =
      await tier1KnowledgeEnhancementEngine.conversationEngine.getPatternsForTopic(topic);
...
const serviceConfig = [
  {
    name: 'training',
    prefixes: ['/api/v1/training/hyper-speed', '/api/v1/training', '/api/v1/next-domain'],
  ...
  svc.prefixes.forEach((prefix) => {
    ...
    app.all([prefix, `${prefix}/*`], async (req, res) => {
      try {
        ...
        console.error(`[Proxy] Error forwarding to ${svc.name} (${prefix}):`, error.message);
        if (!res.headersSent) {
          res.status(502).json({
            ok: false,
...
app.post('/api/v1/activity/heartbeat', (req, res) => {
  ...
  if (sessionId) {
    activitySessions.set(sessionId, { lastSeen: Date.now(), route, action, active: true });
  }
  res.json({
    ok: true,
...
app.post('/api/v1/activity/ensure-real-data', (req, res) => {
  realDataMode = true;
  res.json({ ok: true, realDataMode: true, message: 'Real data mode enabled (integrated)' });
});
...
app.get('/api/v1/activity/servers', async (req, res) => {
  ...
  res.json({ ok: true, servers: status, activeServers: activeCount, totalServers: ports.length });
});
...
app.post('/api/v1/work/request', async (req, res) => {
  try {
    ...
    const submission = workbenchOrchestrator.submitWork(goal, context || {}, options || {});
...
app.post('/api/v1/github/create-pr', async (req, res) => {
  ...
  if (!title || !head)
    return res.status(400).json({ ok: false, error: 'title and head branch required' });
...
app.post('/api/v1/github/comment', async (req, res) => {
  ...
  if (!number || !body)
    return res.status(400).json({ ok: false, error: 'issue/PR number and body required' });
...
app.post('/api/v1/system/self-patch', async (req, res) => {
  try {
    ...
    if (!action) {
      return res
        .status(400)
        .json({ ok: false, error: 'action required (update, create, or analyze)' });
    }
    ...
    if (action === 'analyze') {
      ...
      if (!filePath)
        return res.status(400).json({ ok: false, error: 'filePath required for analysis' });
    ...
    async function buildStructure(dir, depth = 0) {
      ...
      try {
        ...
        for (const file of files) {
          ...
          if (file.startsWith('.') || ['node_modules', 'dist', 'build', '.git'].includes(file))
            continue;
...
app.post('/api/v1/capabilities/activate/phase', async (req, res) => {
  ...
  if (!plan[phase]) {
    return res
      .status(400)
      .json(formatterIntegration.formatErrorResponse(new Error(`Unknown phase: ${phase}`), 400));
  }
...
(async () => {
  try {
    ...
    console.log(
      ...
      aggregation.instructions.getAggregationConfig().aggregationStrategy.description
    );
  } catch (err) {
