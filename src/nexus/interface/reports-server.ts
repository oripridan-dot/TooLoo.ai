...
import {
  generateSmartLLM,
  getProviderStatus,
} from "../../precog/providers/llm-provider.js";
import CostCalculator from "../../precog/engine/cost-calculator.js";
...
import { setupScorecardRoutes } from "../../legacy_migration/servers/provider-scorecard.js";
...
const serviceBreakers = {
  training: new CircuitBreaker("training", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
  meta: new CircuitBreaker("meta", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
  budget: new CircuitBreaker("budget", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
  coach: new CircuitBreaker("coach", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
  segmentation: new CircuitBreaker("segmentation", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
  capabilities: new CircuitBreaker("capabilities", {
    failureThreshold: 3,
    resetTimeoutMs: 20000,
  }),
};
...
async function fetchService(service, endpoint) {
  try {
    ...
    const response = await breaker.execute(
      () =>
        retry(() => fetch(`${SERVICES[service]}${endpoint}`), {
          maxAttempts: 2,
          backoffMs: 100,
          timeout: 5000,
        }),
...
function suggestActions(gaps) {
  const actions = [];
  const add = (priority, area, action, impact) =>
    actions.push({ priority, area, action, impact });
  // Thresholds
  if (gaps.reasoning > 0.05)
    add(
      "high",
      "reasoning",
...
function generateInsights(data) {
  ...
  if (data.capabilities?.total > 0) {
    insights.push({
      ...
      details:
        "Potential for significant capability expansion through method activation",
    });
  }
    ...
    const report = {
      ...
      summary: {
        ...
        milestones: {
          ...
          metaLearning:
            meta?.report?.completedPhases >= 4 ? "completed" : "partial",
      ...
      recommendations: [
        {
          ...
          nextSteps: [
            ...
            "Create capability integration plan",
            "Implement gradual activation",
          ],
        },
        {
          priority: "medium",
...
app.get("/api/v1/reports/delta", async (req, res) => {
  try {
    ...
    const files = (await fs.readdir(outDir))
      .filter((f) => f.startsWith("ai-comparison-") && f.endsWith(".json"))
      .sort();
    if (files.length < 2)
      return res.json({
        ok: true,
        note: "Need at least two comparison snapshots for delta",
      });
...
app.post("/api/v1/reports/ai-external-critique", async (req, res) => {
  try {
    ...
    const metaSummary = meta?.metrics?.current
      ? `LV:${Math.round((meta.metrics.current.learningVelocity || 0) * 100)} AS:${Math.round((meta.metrics.current.adaptationSpeed || 0) * 100)} KR:${Math.round((meta.metrics.current.knowledgeRetention || 0) * 100)} TE:${Math.round((meta.metrics.current.transferEfficiency || 0) * 100)}`
      : "n/a";
    const budgetSummary =
      budget?.limit !== undefined
        ? `Used:${budget.used}/${budget.limit} (${Math.round((budget.percent || 0) * 100)}%)`
        : "n/a";
    const segSummary = seg?.ok
      ? `labels:${(seg.labels || []).length}, components:${Object.keys(seg.components || {}).length}`
      : "n/a";
...
`;
    ...
    if (!anyAvailable && !allowMock) {
      critiques.push({
        ...
        provider: "none",
        error:
          "No external AI providers configured; add API keys or enable local providers to gather external critiques.",
      });
      ...
      for (let i = 0; i < roundsClamped; i++) {
        ...
        try {
          ...
          critiques.push({ ok: false, level, error: String(e.message || e) });
        }
      }
    } else {
      ...
      for (let i = 0; i < roundsClamped; i++) {
        ...
        try {
          ...
          critiques.push({
            ok: true,
            level,
            provider: j.providersAvailable ? "mixed" : "mock",
            text,
            cached: j.cached,
          });
        } catch (e) {
    ...
    res.json({
      ...
      note: anyAvailable
        ? "Critiques aggregated; parse client-side JSON if providers returned structured output."
        : allowMock
          ? "Using mock via provider burst for validation."
          : "No providers available; returned guidance only.",
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
...
app.get("/api/v1/reports/performance", async (req, res) => {
  try {
    ...
    const windowSize =
      isFinite(windowParam) && windowParam > 0
        ? Math.min(500, windowParam)
        : 30;
    ...
    if (component && component !== "all")
      filtered = filtered.filter((p) => p.component === component);
    ...
    const progressSeries = filtered.map((p) => {
      ...
      const progress =
        total > 0 ? Math.min(100, Math.round((cumSuccess / total) * 100)) : 0;
    ...
    const recentSuccessRate =
      recentSlice.length > 0
        ? Math.round((recentOk / recentSlice.length) * 100)
        : 0;
    const summary = {
      ...
      componentProgress:
        compTotals && compTotals.discovered > 0
          ? Math.round((compTotals.activated / compTotals.discovered) * 100)
          : compTotals
            ? 0
            : undefined,
    };
    res.json({
      ...
      summary,
      points: filtered,
      series: { progress: progressSeries, successRate: successRateSeries },
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
        ...
        projectedNext: [
          {
            ...
            estimated: "2025-10-13",
            goal: "Activate 242 discovered methods",
          },
          {
            phase: "Production Readiness",
            estimated: "2025-10-14",
            goal: "Real conversation processing",
          },
          {
            phase: "ML Enhancement",
            estimated: "2025-10-15",
            goal: "Semantic understanding upgrade",
          },
        ],
...
app.get("/api/v1/reports/capabilities", async (req, res) => {
  try {
    ...
    const capabilities =
      meta?.report?.phases?.[1]?.findings?.[1]?.learningTargets || [];
    ...
    if (analysis.componentBreakdown.autonomousEvolutionEngine) {
      analysis.potentialImpact.push({
        ...
        benefit:
          "Automatic performance improvements without manual intervention",
        riskLevel: "medium",
      });
    }
      ...
      insights: generateInsights({
        ...
        capabilities: meta?.report?.phases?.[1]?.findings?.[1]
          ? {
              ...
              components:
                meta.report.phases[1].findings[1].learningTargets?.length || 0,
            }
          : null,
...
app.get("/api/v1/reports/critiques/analysis", async (req, res) => {
  try {
            ...
            if (jsonMatch) {
              ...
              if (parsed.gaps) {
                parsed.gaps.forEach((gap) => {
                  ...
                  gapsByArea[area].avgSeverity =
                    (gapsByArea[area].avgSeverity *
                      (gapsByArea[area].count - 1) +
                      (gap.severity || 0)) /
                    gapsByArea[area].count;
                });
              }
    ...
    const timeline = critiques.map((c) => ({
      ...
      providers:
        c.critiques?.filter((cr) => cr.ok).map((cr) => cr.provider) || [],
      successRate: c.critiques
        ? c.critiques.filter((cr) => cr.ok).length / c.critiques.length
        : 0,
    }));
  ...
  try {
    ...
    res.json({
      ...
      budgetStatus: {
        ...
        status:
          (metrics.budgetUtilization || 0) > 90
            ? "at-capacity"
            : (metrics.budgetUtilization || 0) > 70
              ? "high-usage"
              : (metrics.budgetUtilization || 0) > 30
                ? "moderate"
                : "low-usage",
      },
      costMetrics: {
      ...
      providerRecommendations:
        policy?.providerRecommendations?.slice(0, 3) || [],
...
app.post("/api/v1/reports/analyze", async (req, res) => {
  try {
    ...
    metrics.forEach((m) => {
      AnalyticsEngine.recordMetric(m.name, m.value, {
        provider: m.provider,
...
app.get("/api/v1/reports/trends", async (req, res) => {
  try {
    ...
    const metricList = metrics
      ? metrics.split(",")
      : Array.from(AnalyticsEngine.metrics.keys());
...
app.post("/api/v1/reports/compare", async (req, res) => {
  try {
    ...
    if (!Array.isArray(metrics) || metrics.length === 0) {
      return res
        .status(400)
        .json({ ok: false, error: "metrics array required" });
    }
...
app.post("/api/v1/present", async (req, res) => {
  try {
    ...
    if (!providerResponses || Object.keys(providerResponses).length === 0) {
      return res
        .status(400)
        .json({ ok: false, error: "Provider responses required" });
    }
...
app.post("/api/v1/present/batch", async (req, res) => {
  try {
    ...
    if (!Array.isArray(presentations) || presentations.length === 0) {
      return res
        .status(400)
        .json({ ok: false, error: "presentations array required" });
    }
...
app.get("/api/v1/metrics/engagement/:userId", (req, res) => {
  try {
    const { userId } = req.params;
    const score =
      presentationEngine.metricsCollector?.getEngagementScore(userId);
...
app.get("/api/v1/metrics/progress/:userId", (req, res) => {
  try {
    const { userId } = req.params;
    const progress =
      presentationEngine.metricsCollector?.getProgressMetrics(userId);
...
app.get("/api/v1/badges/user/:userId", (req, res) => {
  try {
    ...
    const totalPoints =
      presentationEngine.badgeSystem?.getUserTotalPoints(userId);
...
app.post("/api/v1/badges/award", (req, res) => {
  try {
    ...
    if (!badge) {
      return res
        .status(400)
        .json({ error: "Badge already awarded or not found" });
    }
...
app.get("/api/v1/badges/rarity-distribution", (req, res) => {
  try {
    const distribution =
      presentationEngine.badgeSystem?.getRarityDistribution();
...
app.get("/api/v1/analytics/summary/:userId", (req, res) => {
  try {
    ...
    res.json({
      ...
      engagementScore,
      totalPoints:
        presentationEngine.badgeSystem?.getUserTotalPoints(userId) || 0,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/api/v1/analytics/dashboard", (req, res) => {
  try {
    const globalMetrics =
      presentationEngine.metricsCollector?.getGlobalMetrics("week");
    const topPerformers =
      presentationEngine.metricsCollector?.getTopPerformers(5);
    const mostAwardedBadges =
      presentationEngine.badgeSystem?.getMostAwardedBadges(5);
    const leaderboard =
      presentationEngine.badgeSystem?.getGlobalLeaderboard(10);
