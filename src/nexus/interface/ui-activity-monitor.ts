...
// Endpoints
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'ui-activity-monitor' }));
...
app.get('/api/v1/activity/sessions', (req, res) => {
  ...
  for (const [id, session] of sessions.entries()) {
    if (now - session.lastSeen < 300000) { // 5 min timeout
      activeSessions.push({ sessionId: id, ...session });
    } else {
      sessions.delete(id);
    }
  }
  res.json({ ok: true, activeSessions: activeSessions.length, sessions: activeSessions });
});
