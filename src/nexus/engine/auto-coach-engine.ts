...
export default class AutoCoachEngine {
  ...
  /**
   * Active Auto-Coach Loop (User Request Integration)
   * Injects strategic context into the provider prompt
   */
  async executeLoop(input) {
    // NEW: Active Memory Injection
    try {
      const response = await axios.post(`${SYNAPSE_URL}/resonance`, {
        context: input.content,
        limit: 5,
      });
      const strategicContext = response.data.data;

      if (strategicContext && strategicContext.length > 0) {
        this.logger.log("TooLoo.ai: Integrating historical partner context...");
        // Inject this context into the provider prompt to ensure continuity
        if (!input.systemPrompts) input.systemPrompts = [];
        input.systemPrompts.push(JSON.stringify(strategicContext));
      }
    } catch (e) {
  ...
  async start() {
    ...
    try {
      await this._maybeAutoFocusBurstOnStart();
    } catch {}
    // Engage Rush Mode by default for Meta‑Learning until completion
    try {
      await this._ensureRushModeForMeta();
    } catch {}
  ...
  async _tick() {
    ...
    const avg = domains.length
      ? domains.reduce((s, d) => s + (d.mastery || 0), 0) / domains.length
      : 0;
    ...
    if (this._metaTriggered) {
      this._lastAction = "meta already triggered";
      return;
    }
    ...
    if (this.thresholds.aggressiveMode && avg < aggressiveThreshold) {
      cycleRounds = Math.floor(
        cycleRounds * (this.thresholds.aggressiveMultiplier || 2),
      );
    ...
    const needsCoaching =
      avg < this.thresholds.minAvgMastery ||
      below80.length > this.thresholds.maxBelow80;
    if (needsCoaching) {
      ...
      for (let mb = 0; mb < micro && ran < cycleRounds; mb++) {
        ...
        const batchPromises = [];
        for (let b = 0; b < toRun; b++) {
          batchPromises.push(this.trainingCamp.runRound());
        }
        const results = await Promise.all(batchPromises);
        this._roundsRun += results.length;
        ran += results.length;
        const modeLabel = isHyperMode
          ? " HYPER"
          : this.thresholds.aggressiveMode && avg < aggressiveThreshold
            ? " aggressive"
            : "";
  ...
  _enableRushMode() {
    ...
    this.thresholds.microBatchesPerTick = Math.max(
      3,
      this.thresholds.microBatchesPerTick || 1,
    );
    this.thresholds.batchSize = Math.max(3, this.thresholds.batchSize || 1);
    this.thresholds.intervalMs = Math.min(
      800,
      this.thresholds.intervalMs || 2000,
    );
  }
  _disableRushMode() {
  ...
  async _ensureRushModeForMeta() {
    try {
      ...
      if (done < 4) this._enableRushMode();
      else this._disableRushMode();
    } catch {
      /* best-effort */
    }
  }
  ...
  async _maybeAutoFocusBurstOnStart() {
    ...
    try {
      ...
      const weak = (overview?.domains || []).filter(
        (d) => (d.mastery || 0) < target - delta,
      );
      if (weak.length > 0) {
        // Temporarily enforce auto-fill with current N; run small burst
        const gaps = Number(sel.gapsCount || 2);
        await fetch(
          `/api/v1/training/settings/update?autoFillGaps=true&gapsCount=${gaps}`,
        );
        const burst = Math.min(6, this.thresholds.maxRoundsPerCycle * 2);
        const promises = [];
        for (let i = 0; i < burst; i++)
          promises.push(this.trainingCamp.runRound());
