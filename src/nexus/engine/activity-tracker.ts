...
export default class ActivityTracker {
  constructor({ storeDir = "data/activity", maxItems = 10 } = {}) {
    ...
    if (!fs.existsSync(this.storeFile))
      fs.writeFileSync(this.storeFile, JSON.stringify({ items: [] }, null, 2));
  }
  _read() {
    try {
  ...
  _write(d) {
    try {
      fs.writeFileSync(this.storeFile, JSON.stringify(d, null, 2));
    } catch {}
  }
  add(item) {
    const d = this._read();
  ...
  getSummary() {
    ...
    const avgLatency =
      items.reduce((s, x) => s + (x.meta?.latencyMs || 0), 0) / n;
    const byProvider = items.reduce((m, x) => {
      const p = x.providerUsed || "unknown";
      m[p] = (m[p] || 0) + 1;
      return m;
    }, {});
