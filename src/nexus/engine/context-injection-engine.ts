...
class ContextInjectionEngine {
  ...
  buildContextInjection(memoryEngine, currentMessage, options = {}) {
    ...
    if (strategy === "relevance") {
      contextMessages = this._buildRelevanceContext(
        memoryEngine,
        currentMessage,
      );
    ...
    const injection = this._formatContextInjection(
      contextMessages,
      memoryEngine,
    );
  ...
  _buildRelevanceContext(memoryEngine, currentMessage) {
    ...
    const scored = history.map((msg, _idx) => {
      const relevanceScore = this._scoreMessageRelevance(
        msg.content,
        intent,
        keywords,
      );
  ...
  _scoreMessageRelevance(messageContent, intent, keywords) {
    ...
    if (intent === "question" && lower.match(/answer|explains?|here|this/i)) {
      ...
      intent === "task" &&
      lower.match(/implement|code|function|method/i)
    ) {
      score += 0.2;
    } else if (
      intent === "debugging" &&
      lower.match(/error|issue|problem|fix/i)
    ) {
      score += 0.2;
    }
