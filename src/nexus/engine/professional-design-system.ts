...
class ProfessionalDesignSystem {
  ...
  public ingestArtifacts(
    dir: string = path.join(process.cwd(), "data", "design-artifacts"),
  ) {
    ...
    try {
      ...
      this.artifacts = files.filter((f) =>
        /\.(png|jpg|jpeg|svg|webp)$/i.test(f),
      );
      console.log(
        `[DesignSystem] Ingested ${this.artifacts.length} artifacts from ${dir}`,
      );
    } catch (e) {
  ...
  mapDesignSystemToTokens(designInput: Record<string, any>) {
    ...
    if (designInput.colors) {
      ...
      tokens.colors.tertiary =
        designInput.colors.tertiary || designInput.colors.secondary;
  ...
  generateComprehensiveCSS(tokens: Record<string, any>) {
    let css =
      "/* AUTO-GENERATED COMPREHENSIVE COMPONENT CSS FROM DESIGN TOKENS */\n\n";
    ...
    Object.entries(this.componentMap).forEach(
  ...
  getComponentsByCategory(category: string): string[] {
    const categories: Record<string, string[]> = {
      buttons: Object.keys(this.componentMap).filter((c) =>
        c.includes("button"),
      ),
      forms: Object.keys(this.componentMap).filter((c) =>
        ["input", "textarea", "select", "checkbox", "radio", "toggle"].includes(
          c,
        ),
      ),
      navigation: Object.keys(this.componentMap).filter(
        (c) =>
          ["nav", "breadcrumb", "pagination", "tabs"].includes(c) ||
          c.includes("nav."),
      ),
      containers: Object.keys(this.componentMap).filter((c) =>
        ["card", "panel", "section"].includes(c),
      ),
      typography: Object.keys(this.componentMap).filter(
        (c) =>
          c.includes("heading") || c.includes("body") || c.includes("caption"),
      ),
      feedback: Object.keys(this.componentMap).filter((c) =>
        ["alert", "badge", "tooltip", "modal", "notification"].includes(c),
      ),
      data: Object.keys(this.componentMap).filter(
        (c) =>
          ["table", "list", "avatar", "progress"].includes(c) ||
          c.includes("table.") ||
          c.includes("list."),
      ),
      interactive: Object.keys(this.componentMap).filter(
        (c) =>
          ["accordion", "dropdown", "popover"].includes(c) ||
          c.includes("accordion.") ||
          c.includes("tabs."),
      ),
    };
  ...
  getSystemStats() {
    return {
      ...
      totalTokens: Object.keys(this.tokenCategories).reduce((sum, cat) => {
        ...
        return (
          sum +
          (typeof catTokens === "object" ? Object.keys(catTokens).length : 1)
        );
      }, 0),
