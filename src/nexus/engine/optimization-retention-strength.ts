...
export function calculateRecencyWeightedReuse(conversations) {
  ...
  const reuseScore =
    maxPossibleWeight > 0 ? totalWeightedUse / maxPossibleWeight : 0;
...
export function analyzeLongTermEngagement(conversations) {
  ...
  const third3 = conversations.slice(
    Math.floor((2 * conversations.length) / 3),
  );
...
export function calculateChurnRisk(reuseScore, halfLife, trajectory) {
  ...
  return {
    ...
    riskLevel:
      churnRisk > 0.7 ? "critical" : churnRisk > 0.4 ? "high" : "moderate",
  };
}
...
function generateRealisticConversations(retentionType, weeks = 12) {
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    const interactions =
      retentionType === "highRetention"
        ? Math.floor(25 + w * 0.5) // Steady or growing
        : Math.floor(30 - w * 0.8); // Declining

    for (let i = 0; i < interactions; i++) {
      const capId =
        Math.random() < reuseRate
          ? reusableCapabilities[
              Math.floor(Math.random() * reusableCapabilities.length)
            ]
...
async function main() {
  ...
  const highRetentionConvs = generateRealisticConversations(
    "highRetention",
    12,
  );
  ...
  const highChurn = calculateChurnRisk(
    highReuseScore,
    highHalfLife.halfLife,
    highEngagement.trajectory,
  );
  const highROI = calculateRetentionROI(
    highReuseScore,
    highEngagement.trajectory,
  );
  ...
  console.log(
    `  Churn Risk: ${(highChurn.churnRisk * 100).toFixed(0)}% (${highChurn.riskLevel})`,
  );
  ...
  const lowChurn = calculateChurnRisk(
    lowReuseScore,
    lowHalfLife.halfLife,
    lowEngagement.trajectory,
  );
  ...
  console.log(
    `  Churn Risk: ${(lowChurn.churnRisk * 100).toFixed(0)}% (${lowChurn.riskLevel})`,
  );
