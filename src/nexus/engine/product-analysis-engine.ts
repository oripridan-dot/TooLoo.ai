    ...
    try {
      ...
      validResponses.forEach((resp) => {
        resp.ideas.forEach((idea) => {
          const key =
            idea.name?.toLowerCase().replace(/\s+/g, "-") || "unnamed";
          if (!ideasMap.has(key)) {
            ideasMap.set(key, {
              ...idea,
            ...
            existing.avgConfidence =
              (existing.avgConfidence + resp.confidence) / 2;
          }
        });
      });
...
Return as valid JSON array with scores and analysis.`;
    ...
    return scores.map((score, idx) => ({
      ...
      consensusScore:
        score.scores && score.scores.length > 0
          ? (
              score.scores.reduce((a, b) => a + b, 0) / score.scores.length
            ).toFixed(1)
          : 0,
  ...
  aggregateScores(responses, ideas) {
    ...
    ideas.forEach((idea, idx) => {
      const ideaName =
        typeof idea === "string" ? idea : idea.name || `Idea ${idx}`;
