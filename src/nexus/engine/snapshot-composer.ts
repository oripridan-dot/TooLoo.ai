  ...
  const traitVector = traits
    ? {
        decisionCompression: {
          ...
          interpretation: getTraitInterpretation(
            "decisionCompression",
            traits.decisionCompression,
          ),
        },
        riskDiscipline: {
...
function getConversationStyle(patterns) {
  ...
  const structurePatterns = patterns.filter(
    (p) => p.id && p.id.includes("structure"),
  );
  const riskPatterns = patterns.filter((p) => p.id && p.id.includes("risk"));

  // Calculate style scores
  const pivotScore = pivotPatterns.reduce(
    (sum, p) => sum + (p.confidence || 0),
    0,
  );
  const structureScore = structurePatterns.reduce(
    (sum, p) => sum + (p.confidence || 0),
    0,
  );
  const riskScore = riskPatterns.reduce(
    (sum, p) => sum + (p.confidence || 0),
    0,
  );
