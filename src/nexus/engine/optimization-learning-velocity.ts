...
export function calculateOptimizedVelocity(conversations) {
  ...
  weekArray.forEach((entry, idx) => {
    const rawVelocity = Math.min(
      entry.capabilityCount / Math.max(entry.interactionCount / 5, 1),
      1.0,
    );
    const weekAge =
      (now - new Date(entry.week).getTime()) / (1000 * 60 * 60 * 24 * 7);
...
export function analyzeVelocityTrend(conversations) {
  ...
  if (Math.abs(delta) < 0.05) {
    ...
    return {
      trend: "improving",
      confidence: Math.min(Math.abs(delta) * 2, 1.0),
    };
  } else {
    return {
      trend: "declining",
...
function generateRealisticConversations(userType, weeks = 12) {
  ...
  for (let w = 0; w < weeks; w++) {
    ...
    const capsPerWeek = isFastLearner
      ? 5 + Math.floor(Math.random() * 4)
      : 1 + Math.floor(Math.random() * 2);
    const interactionsPerCap = isFastLearner
...
async function main() {
  ...
  console.log(
    `  ✓ Regular learner: ${regularLearnerConvs.length} interactions`,
  );

  console.log("\n📈 Step 3: Fast Learner Classification");
  const fastResult = classifyLearnerSpeed(fastLearnerConvs);
  const fastROI = calculateOptimizedROIMultiplier(
    fastResult.velocity,
    fastResult.trend,
  );
  ...
  const regularROI = calculateOptimizedROIMultiplier(
    regularResult.velocity,
    regularResult.trend,
  );
