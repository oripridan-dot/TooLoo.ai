...
class AutomatedLearningPipelineExtensions {
  ...
  async testPatternAccuracy(pattern, trainingData) {
    ...
    for (const conversation of relevantConversations.slice(0, 50)) {
      // Test on 50 conversations max
  ...
  calculatePatternSimilarity(pattern1, pattern2) {
    ...
    const intersection = new Set(
      [...indicators1].filter((x) => indicators2.has(x)),
    );
  ...
  async testPatternStability(pattern, trainingData) {
    ...
    for (const type of conversationTypes) {
      const typeConversations = trainingData.filter(
        (conv) => conv.metadata.scenario === type,
      );
    ...
    return conversationTypes.length > 0
      ? successfulTypes / conversationTypes.length
      : 0;
  }
  ...
  async measurePerformanceGain(validatedPatterns) {
    ...
    const newPerformance = await this.measurePerformanceWithNewPatterns(
      testData,
      validatedPatterns,
    );
    ...
    console.log(
      `📊 Baseline: ${Math.round(baselinePerformance * 100)}%, New: ${Math.round(newPerformance * 100)}%, Improvement: ${Math.round(improvement * 100)}%`,
    );
  ...
  async measureCurrentPerformance(testData) {
    ...
    for (const conversation of testData) {
      try {
        ...
        const patterns = this.runEnhancedPatternExtraction(
          conversation.messages,
        ...
        console.warn(
          "⚠️  Performance test failed for conversation:",
          error.message,
        );
      }
    }
  ...
  scoreAnalysisQuality(patterns, traits, conversation) {
    ...
    if (conversation.metadata.targetPatterns) {
      ...
      const expectedDetected = expectedPatterns.filter((p) =>
        detectedIds.includes(p),
      ).length;
...
`;
  ...
  async updatePatternDefinitions(newPatterns) {
    ...
    console.log(
      `📝 Updated pattern definitions with ${newPatterns.length} new patterns`,
    );
  }
