...
export default class SystemOperationsManager {
  ...
  async writeProbe() {
    try {
      fs.mkdirSync("data/ops", { recursive: true });
      fs.writeFileSync(
        "data/ops/probe.json",
  ...
  async runSmoke() {
    ...
    try {
      const r = await fetch(`${this.baseUrl}/api/v1/extract/structured`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: "Accuracy 92%, latency 45ms, $0.25 per 1K tokens",
        }),
      });
      extractor = { ok: r.ok };
    } catch (e) {
      extractor = { ok: false, error: e.message };
    }
    // ingestion docs list
    const webDocs = await this.ping("/api/v1/web/docs?taskId=general&limit=1");
    const ok =
      [checks.health, checks.segStatus, checks.alerts, extractor].every(
        (x) => x && x.ok,
      ) && wrote;
    const status = {
      ok,
      checks: { ...checks, extractor, wroteProbe: wrote, webDocs },
    };
