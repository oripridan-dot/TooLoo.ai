...
class RealTimeLearningEngine {
  constructor() {
    ...
    this.performanceFile = path.join(
      process.cwd(),
      "logs",
  ...
  recordValidationOutcome({
    ...
    const outcome = {
      ...
      effective:
        effective !== null
          ? effective
          : agreement > this.dynamicThresholds.agreementThreshold,
    };
  ...
  updateRealTimeMetrics(outcome) {
    ...
    taskStats.successRate =
      (taskStats.successRate * (taskStats.count - 1) +
        (outcome.effective ? 1 : 0)) /
      taskStats.count;
    taskStats.avgAgreement =
      (taskStats.avgAgreement * (taskStats.count - 1) + outcome.agreement) /
      taskStats.count;
    taskStats.avgCost =
      (taskStats.avgCost * (taskStats.count - 1) + outcome.costIncurred) /
      taskStats.count;
  ...
  adaptThresholds() {
    console.log(
      "🧠 Real-time learning: Adapting thresholds based on recent performance...",
    );

    const recentOutcomes = this.performanceHistory.slice(
      -this.adaptationInterval,
    );
    if (recentOutcomes.length < this.minSampleSize) {
      console.log(
        "📊 Insufficient data for adaptation (need at least",
        this.minSampleSize,
        "samples)",
      );
      return;
    ...
    const successRate =
      recentOutcomes.filter((o) => o.effective).length / recentOutcomes.length;
    const avgAgreement =
      recentOutcomes.reduce((sum, o) => sum + o.agreement, 0) /
      recentOutcomes.length;
    const avgCost =
      recentOutcomes.reduce((sum, o) => sum + o.costIncurred, 0) /
      recentOutcomes.length;
    const avgConfidence =
      recentOutcomes.reduce((sum, o) => sum + o.originalConfidence, 0) /
      recentOutcomes.length;
    ...
    for (const [threshold, change] of Object.entries(adaptations)) {
      if (this.dynamicThresholds.hasOwnProperty(threshold)) {
        ...
        // Apply bounds
        this.dynamicThresholds[threshold] = this.boundThreshold(
          threshold,
          newValue,
        );
        ...
        if (Math.abs(change) > 0.1) {
          console.log(
            `🔧 Adapted ${threshold}: ${oldValue.toFixed(1)} → ${this.dynamicThresholds[threshold].toFixed(1)}`,
          );
        }
      }
    }
  ...
  updateGlobalConfig() {
    ...
    if (global.qualityGatesConfig) {
      global.qualityGatesConfig.confidenceThreshold = Math.round(
        this.dynamicThresholds.confidenceThreshold,
      );
      global.qualityGatesConfig.agreementThreshold = Math.round(
        this.dynamicThresholds.agreementThreshold,
      );
  ...
  getAdaptiveMetrics() {
    ...
    return {
      ...
      recentPerformance: {
        ...
        successRate:
          recentOutcomes.length > 0
            ? Math.round(
                (recentOutcomes.filter((o) => o.effective).length /
                  recentOutcomes.length) *
                  100,
              )
            : 0,
      ...
      adaptationHistory: {
        ...
        nextAdaptationIn:
          this.adaptationInterval -
          (this.validationCount % this.adaptationInterval),
      },
      taskTypePerformance: Array.from(
        this.performanceMetrics.taskTypes.entries(),
      ).map(([type, stats]) => ({
        taskType: type,
        count: stats.count,
