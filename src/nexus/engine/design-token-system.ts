...
export class DesignTokenSystem {
  constructor() {
    ...
    this.tokenDefinitions = {
      ...
      "color-primary-hover": {
        category: "color",
        semantic: "primary hover state",
      },
      "color-primary-active": {
        category: "color",
        semantic: "primary active state",
      },
  ...
  mapDesignSystemToTokens(designSystem) {
    ...
    if (colors) {
      const colorArray = Object.entries(colors)
        .filter(
          ([, v]) => typeof v === "string" && v.match(/^#[0-9a-fA-F]{6}$/i),
        )
      ...
      if (colorArray.length > 0) {
        ...
        const primary =
          colorArray.find((c) => c.name.includes("primary")) || colorArray[0];
        ...
        // Secondary color
        const secondary =
          colorArray.find((c) => c.name.includes("secondary")) || colorArray[1];
        if (secondary) {
          mappedTokens["color-secondary"] = secondary.value;
          mappedTokens["color-secondary-dark"] = this.darken(
            secondary.value,
            20,
          );
          mappedTokens["color-secondary-hover"] = this.lighten(
            secondary.value,
            10,
          );
        }
        ...
        // Surface colors
        const background = colorArray.find(
          (c) => c.name.includes("bg") || c.name.includes("background"),
        );
        const text = colorArray.find(
          (c) => c.name.includes("text") || c.name.includes("foreground"),
        );

        if (background) {
          mappedTokens["color-surface"] = background.value;
          mappedTokens["color-surface-secondary"] = this.lighten(
            background.value,
            5,
          );
        }
        ...
        if (text) {
          ...
          mappedTokens["color-text-inverse"] = this.isLight(text.value)
            ? "#000000"
            : "#ffffff";
        ...
        const error = colorArray.find(
          (c) => c.name.includes("error") || c.name.includes("danger"),
        );
        ...
        // Borders
        mappedTokens["color-border"] = this.lighten(
          text?.value || "#ffffff",
          70,
        );
        mappedTokens["color-border-subtle"] = this.lighten(
          text?.value || "#ffffff",
          80,
        );
      }
    ...
    // Map typography
    if (typography) {
      const fonts = Object.values(typography).filter(
        (f) => typeof f === "string",
      );
      if (fonts.length > 0) {
  ...
  generateComponentCSS() {
    ...
    Object.entries(this.componentTokenMap).forEach(
      ([componentSelector, tokenUsage]) => {
        const rules = {};
        ...
        Object.entries(tokenUsage).forEach(([cssProperty, tokenKey]) => {
          ...
          if (tokenValue) {
            ...
            const actualCSSProperty = this.mapPropertyToCSS(
              cssProperty,
              componentSelector,
            );
