...
export class CrossProviderValidator {
  ...
  async consensusValidation({
    ...
    const results = await Promise.allSettled(
      this.providers
        .slice(0, minProviders)
        .map((provider) => this.callWithMetrics(prompt, provider)),
    );
    ...
    if (responses.length < minProviders) {
      throw new Error(
        `Insufficient providers responded (${responses.length}/${minProviders})`,
      );
    }
  ...
  async verificationChain({
    ...
    if (this.shouldValidate(primary)) {
      ...
      primary.validations = validations.map((v) =>
        v.status === "fulfilled" ? v.value : null,
      );
      primary.confidence = this.calculateValidatedConfidence(
        primary,
        validations,
      );
    }
  ...
  async qualityGateValidation({ prompt, taskType, threshold = 0.8 }) {
    const primary = await this.callWithMetrics(
      prompt,
      this.selectPrimaryProvider(taskType),
    );
  ...
  shouldValidate(response) {
    return (
      (response.text.length > 100 && response.confidence < 0.8) ||
  ...
  estimateConfidence(text, provider) {
    ...
    const providerConfidence = { deepseek: 0.75, claude: 0.85, gemini: 0.8 };
  ...
  calculateAgreement(responses) {
    ...
    for (let i = 0; i < responses.length; i++) {
      for (let j = i + 1; j < responses.length; j++) {
        similarities.push(
          this.textSimilarity(responses[i].text, responses[j].text),
        );
      }
    }
