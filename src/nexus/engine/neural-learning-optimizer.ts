...
class NeuralLearningOptimizer {
  ...
  initializeWeights() {
    ...
    console.log(
      "🧠 Neural Learning Optimizer initialized with",
      strategies.length,
      "strategies",
    );
  }
  ...
  calculateStrategyScore(strategy, weights, state) {
    const { avgMastery, weakestDomain, masteredDomains, trainingRounds } =
      state;

    let score =
      weights.efficiency * 0.4 +
      weights.momentum * 0.3 +
      weights.adaptability * 0.3;
  ...
  updateWeights(strategy, reward, state) {
    ...
    const prediction = this.calculateStrategyScore(
      strategy,
      currentWeights,
      state,
    );
    const target =
      reward + this.discountFactor * this.getMaxFutureReward(state);
  ...
  async executeOptimizedLearning(rounds = 50) {
    ...
    console.log(
      "🔄 Using reinforcement learning to discover best strategies\n",
    );
    ...
    for (let round = 1; round <= rounds; round++) {
      ...
      strategyUsage.set(
        selectedStrategy,
        (strategyUsage.get(selectedStrategy) || 0) + 1,
      );
      ...
      if (round % 10 === 0) {
        ...
        console.log(
          `Round ${round}: Avg mastery ${currentState.avgMastery.toFixed(1)}% | Strategy: ${selectedStrategy} | Reward: ${reward.toFixed(2)}`,
        );
      }
    }
    ...
    const sortedStrategies = Array.from(strategyUsage.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    sortedStrategies.forEach(([strategy, count], index) => {
      ...
      const effectiveness =
        (weights.efficiency + weights.momentum + weights.adaptability) / 3;
      const medal =
        index === 0 ? "🥇" : index === 1 ? "🥈" : index === 2 ? "🥉" : "  ";
      console.log(
        `${medal} ${strategy}: ${count} uses, ${(effectiveness * 100).toFixed(1)}% effectiveness`,
      );
    });
  ...
  async getCurrentLearningState() {
    try {
      ...
      if (data.ok && data.data) {
        ...
        const weakestDomain = domains.reduce((min, d) =>
          d.mastery < min.mastery ? d : min,
        );
  ...
  calculateReward(result, previousState) {
    ...
    const newMasteredDomains =
      newState.masteredDomains - previousState.masteredDomains;
