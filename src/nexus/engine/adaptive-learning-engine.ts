...
class AdaptiveLearningEngine {
  constructor(trainingCamp) {
    ...
    console.log(
      "📊 Testing multiple learning strategies to find optimal approach",
    );
  }
  ...
  analyzeTrainingEffectiveness() {
    ...
    return {
      highResponders: effectiveness
        .filter((e) => e.effectiveness > 5)
        .map((e) => e.topic),
  ...
  calculateConsistency(scores) {
    ...
    const diffs = scores
      .slice(1)
      .map((score, i) => Math.abs(score - scores[i]));
  ...
  async selectOptimalStrategy() {
    ...
    const strategyPerformance = Array.from(
      this.learningStrategies.entries(),
    ).map(([key, strategy]) => {
      const successRate =
        strategy.attempts > 0 ? strategy.success / strategy.attempts : 0;
    ...
    if (shouldExplore && strategyPerformance.length > 1) {
      ...
      selectedStrategy =
        topStrategies[Math.floor(Math.random() * topStrategies.length)];
  ...
  calculateRecentPerformance(strategyKey) {
    ...
    const strategyRounds = recentRounds.filter(
      (round) => round.strategy === strategyKey,
    );

    if (strategyRounds.length === 0) return 0;

    const avgImprovement =
      strategyRounds.reduce((sum, round) => sum + round.improvement, 0) /
      strategyRounds.length;
  ...
  async executeAdaptiveTraining() {
    ...
    for (let i = 0; i < currentStrategy.batchSize; i++) {
      try {
        const result = await this.executeStrategyRound(
          targets,
          currentStrategy,
        );
        results.push(result);
        console.log(
          `  Round ${i + 1}: ${result.trained.join(", ")} (avg improvement: ${result.avgImprovement.toFixed(1)}%)`,
        );
      } catch (error) {
    ...
    if (improvement > 2) {
      // Consider success if improvement > 2%
    ...
    console.log(
      `🏆 Strategy success rate: ${((currentStrategy.success / currentStrategy.attempts) * 100).toFixed(1)}%`,
    );
  ...
  async executeStrategyRound(targets, strategy) {
    ...
    const response = await fetch(
      "http://localhost:3001/api/v1/training/round",
      {
        method: "POST",
  ...
  async getOverview() {
    const response = await fetch(
      "http://localhost:3001/api/v1/training/overview",
    );
    const data = await response.json();
  ...
  calculateAverageImprovement(overview, targets) {
    ...
    const avgMastery =
      targetDomains.reduce((sum, d) => sum + d.mastery, 0) /
  ...
  async runContinuousAdaptiveLearning(maxRounds = 50) {
    ...
    console.log(
      "🎯 Goal: Find optimal learning strategy through experimentation\n",
    );
    ...
    for (let round = 1; round <= maxRounds; round++) {
      ...
      try {
        ...
        if (round % 10 === 0) {
          ...
          if (overview) {
            const avgMastery =
              overview.domains.reduce((sum, d) => sum + d.mastery, 0) /
              overview.domains.length;
            const below80 = overview.domains.filter(
              (d) => d.mastery < 80,
            ).length;
  ...
  generateLearningReport() {
    ...
    const strategies = Array.from(this.learningStrategies.entries()).map(
      ([key, strategy]) => ({
        name: strategy.name,
        successRate:
          strategy.attempts > 0
            ? ((strategy.success / strategy.attempts) * 100).toFixed(1)
            : "0.0",
    ...
    strategies.forEach((strategy, index) => {
      ...
      console.log(
        `${medal} ${strategy.name}: ${strategy.successRate}% success (${strategy.attempts} attempts)`,
      );
    ...
    console.log(
      "   This strategy should be used for future training sessions.",
    );

    console.log("\n💡 LEARNING INSIGHTS:");
    if (parseFloat(bestStrategy.successRate) > 70) {
      console.log(
        `   ✅ Found highly effective learning approach (${bestStrategy.successRate}%)`,
      );
    } else if (parseFloat(bestStrategy.successRate) > 50) {
      console.log(
        `   📈 Discovered moderately effective approach (${bestStrategy.successRate}%)`,
      );
    } else {
