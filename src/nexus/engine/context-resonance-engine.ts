...
export default class ContextResonanceEngine {
  constructor(options = {}) {
    ...
    this.memoryFile = path.join(this.workspaceRoot, 'data', 'segmentation-memory.json');
  ...
  async retrieveResonantMemory(currentContext, traits = []) {
    ...
    const weightedMemories = await Promise.all(rawSegments.map(async (segment) => {
...
`;
  ...
  async saveSegments(segments) {
    try {
      await fs.promises.writeFile(this.memoryFile, JSON.stringify(segments, null, 2));
    } catch (e) {
