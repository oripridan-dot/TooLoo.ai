...
export default class PerMetricStrategies {
  ...
  private history: any[];
  private effectiveness: Record<string, any>;
   ...
   */
  selectStrategy(metric: string, state: string, trend: string) {
    ...
    const scored = Object.entries(available).map(([key, strategy]: [string, any]) => {
      let score = 0;
   ...
   */
  analyzeMetricState(value: number) {
   ...
   */
  calculateTrendDirection(deltas: number[]) {
    if (!deltas || deltas.length < 3) return 'unknown';

    // Get recent vs previous averages
    const recent = deltas.slice(-3).reduce((a: number, b: number) => a + b, 0) / 3;
    const threshold =
      deltas.slice(-6, -3).length > 0
        ? deltas.slice(-6, -3).reduce((a: number, b: number) => a + b, 0) / 3
        : 0;
   ...
   */
  applyStrategies(metrics: Record<string, number>, trends: Record<string, string>) {
   ...
   */
  trackEffectiveness(strategy: string, metric: string, boost: number, delta: number) {
  ...
  getEffectivenessSummary() {
    ...
    for (const [key, data] of Object.entries(this.effectiveness) as [string, any][]) {
  ...
  getRecentEffectiveness(window = 10) {
    ...
    const summary: {
      ...
      averageBoostPerApplication: number;
      strategiesUsed: Record<string, number>;
    } = {
      applicationsCount: recent.length,
      totalBoostApplied: recent.reduce((sum: number, h: any) => sum + h.totalBoost, 0),
    ...
    if (recent.length > 0) {
      ...
      for (const h of recent) {
        for (const [_metric, result] of Object.entries(h.results) as [string, any][]) {
   ...
   */
  importState(state: { history?: any[]; effectiveness?: Record<string, any> }) {
