...
export default class EmotionDetectionEngine {
  ...
  analyzeEmotion(text) {
    ...
    const confidence = Math.min(
      ...
      Math.max(
        0.3,
        (emotionScores[primary] || 0) / Math.max(1, text.split(" ").length),
      ),
    );
  ...
  assessIntensity(text) {
    ...
    // Intensifier words
    const intensifiers = this.sentimentShifts.intensifiers.filter((i) =>
      text.toLowerCase().includes(i),
    ).length;
    intensity += intensifiers * 0.1;

    // Negations
    const negations = this.sentimentShifts.negations.filter((n) =>
  ...
  generateEmotionalRecommendations(emotion, sentiment, intensity) {
    ...
    if (intensity > 0.8) {
      recommendations.push(
        "High emotional intensity - provide careful, detailed response",
      );
    }
