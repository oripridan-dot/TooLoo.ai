...
export default class CreativeGenerationEngine {
  ...
  generateCreativeVariations(prompt, options = {}) {
    ...
    return {
      ...
      summary: {
        ...
        avgNoveltyScore:
          variations.reduce((sum, v) => sum + v.noveltyScore, 0) /
          variations.length,
    ...
    for (let i = 0; i < variations.length - 1; i++) {
      for (let j = i + 1; j < variations.length; j++) {
        const words1 = new Set(
          variations[i].variation.toLowerCase().split(/\s+/),
        );
        const words2 = new Set(
          variations[j].variation.toLowerCase().split(/\s+/),
        );
  ...
  findDominantTechnique(variations) {
    ...
    return (
      Object.entries(techniques).sort(([, a], [, b]) => b - a)[0]?.[0] ||
      "mixed"
    );
  }
  ...
  iterativeIdeation(prompt, cycles = 3) {
    ...
    return {
      ...
      totalNoveltyGain:
        evolution.reduce((sum, e) => sum + e.noveltyScore, 0) / cycles,
    };
  }
  ...
  brainstormByDomain(
    prompt,
    domains = ["technical", "creative", "practical", "theoretical"],
  ) {
    const results = {};
