...
export class ConfidenceScorer {
  ...
  score(artifact, evidence = {}) {
    ...
    const semanticScore = this.scoreSemantic(
      artifact.content || "",
      evidence.semanticMetrics || {},
    );
    ...
    const reliabilityScore = this.scoreReliability(
      ...
      evidence.historicalSuccess || 0,
    );
    total += reliabilityScore * weights.reliability;

    // Cost penalty (prefer cheaper solutions)
    const costScore = 1 - Math.min(1, (evidence.costUsd || 0) / 0.1); // normalize to $0.10
  ...
  scoreGrounding(sources = [], claims = []) {
    ...
    claims.forEach((claim) => {
      const hasSource = sources.some(
        (s) => s.text && claim.toLowerCase().includes(s.text.toLowerCase()),
      );
  ...
  scoreCritic(agreement = {}) {
    ...
    const scores = Object.values(agreement).filter(
      (v) => typeof v === "number",
    );
  ...
  scoreSemantic(content = "", metrics = {}) {
    ...
    const jargonDensity =
      (content.match(/[^a-zA-Z0-9\s]/g) || []).length / contentLength;
    ...
    if (metrics.relevanceScore !== undefined)
      score += metrics.relevanceScore * 0.1;
