...
export class Layer1Orchestrator {
  ...
  async orchestrateMultiProvider(prompt, system = '', config = {}) {
    ...
    const results = await Promise.allSettled(
      providersToCall.map(providerName =>
        this.callProviderWithTimeout(
          ...
          prompt,
          system,
          { taskType, criticality, maxTokens }
        )
      )
    );
    ...
    results.forEach((result, idx) => {
      if (result.status === 'fulfilled' && result.value) {
        ...
        console.warn(`Provider ${providersToCall[idx]} failed:`, result.reason?.message);
      }
    });
  ...
  selectProviders(count = 3) {
    ...
    const order = this.options.includeLocalProviders
      ? ['ollama', 'localai', 'deepseek', 'anthropic', 'openai', 'gemini']
    ...
    for (const provider of order) {
      const config = {
        ...
        ollama: () => process.env.OLLAMA_ENABLED === 'true' ? provider : null,
        localai: () => process.env.LOCALAI_ENABLED === 'true' ? provider : null
      };
  ...
  async callProviderWithTimeout(providerName, prompt, system, config) {
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error(`Provider ${providerName} timeout (${this.options.timeout}ms)`));
