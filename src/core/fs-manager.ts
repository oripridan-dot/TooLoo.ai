...
export class SmartFSManager {
   ...
   */
  public async cleanRecovery(keepCount: number = 5): Promise<number> {
    let deletedCount = 0;
    try {
  ...
  private scanImports(content: string): string[] {
    ...
    return matches.map((m) => m[1]).filter((p): p is string => p !== undefined);
  }

  private async resolveImportPath(sourceFile: string, importPath: string): Promise<string | null> {
