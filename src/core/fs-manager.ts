  ...
  public async cleanRecovery(keepCount: number = 5): Promise<number> {
    ...
    try {
      ...
      for (const [_, fileList] of backups) {
        ...
        fileList.sort((a, b) => {
          const timeA = parseInt(a.split(".").slice(-2, -1)[0]);
          const timeB = parseInt(b.split(".").slice(-2, -1)[0]);
          return timeB - timeA;
        });
      ...
      console.log(
        `[SmartFS] 🧹 Cleaned ${deletedCount} files from recovery & memory.`,
      );
    } catch (err) {
