...
class ModuleRegistry extends EventEmitter {
    ...
    register(module: SystemModule) {
        ...
        console.log(`[Registry] Module registered: ${module.name} v${module.version} [${module.status}]`);
    }

    updateStatus(name: string, status: SystemModule['status'], meta?: Record<string, any>) {
