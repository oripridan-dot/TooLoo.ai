...
class ServiceRegistry {
  ...
  register<T>(
    ...
    console.log(
      `[ServiceRegistry] ✓ Registered: ${name} (v${version}) → ${category}`
    );
  ...
  getStatusReport(): string {
    ...
    const unregistered = SERVICE_CATEGORIES.filter(
      (cat) => !this.services.has(cat)
    );
    ...
    for (const [category, reg] of registered) {
      lines.push(
        `  ${category.padEnd(25)} → ${reg.name} (v${reg.version})`
      );
    }
...
export function RegisterService(category: ServiceCategory, version: string = "1.0.0") {
  return function <T extends { new (...args: unknown[]): object }>(constructor: T) {
    ...
    const wrapped = function (this: unknown, ...args: unknown[]) {
      ...
      serviceRegistry.register(category, constructor.name, instance, { version });
      return instance;
    };
