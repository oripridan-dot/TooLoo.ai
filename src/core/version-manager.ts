...
export class VersionManager {
  ...
  private checkGitStatus() {
    this.execGit('rev-parse --abbrev-ref HEAD', (err, branch) => {
      if (!err && branch) {
        ...
        console.log(`[VersionManager] Connected to git branch: ${currentBranch}`);
        ...
        console.warn('[VersionManager] Warning: Not in a git repository or git not available.');
      }
    });
  }
  ...
  private performUpdate() {
    ...
    console.log(`[VersionManager] Processing ${this.pendingChanges.size} file changes...`);
    ...
    console.log(`[VersionManager] Upgraded to v${newVersionString}. Tagged ${taggedCount} files.`);
  ...
  private execGit(command: string, callback: (error: Error | null, stdout?: string) => void) {
  ...
  private tagFile(filePath: string, version: string): boolean {
    try {
      ...
      if (regex.test(content)) {
        ...
        // Insert at top
        if (ext === '.html' && content.toLowerCase().includes('<!doctype html>')) {
          newContent = content.replace(/<!doctype html>/i, `<!DOCTYPE html>\n${tag}`);
        } else {
          newContent = `${tag}\n${content}`;
        }
      }
