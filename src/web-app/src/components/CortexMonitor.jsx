...
const CortexMonitor = ({ compact = false, activeProvider = null }) => {
  ...
  return (
    <div
      className={`${compact ? "h-full w-full flex items-center justify-center overflow-hidden relative" : "bg-[#0f1117] border border-cyan-900/30 rounded-xl p-6 shadow-[0_0_30px_rgba(0,243,255,0.05)] h-full flex flex-col"}`}
    >
      ...
      <div
        className={`flex-1 flex flex-col items-center justify-center ${compact ? "" : "min-h-[200px]"}`}
      >
        <svg viewBox="0 0 400 400" className="w-full h-full overflow-visible">
          ...
          <circle
            ...
            stroke="#00f3ff"
            strokeWidth="2"
            className="filter drop-shadow-[0_0_15px_rgba(0,243,255,0.6)]"
          />
          <text
            x={centerX}
            y={centerY}
          ...
          {providerList.map((p, i) => {
            ...
            const edgeColor = isActive
              ? "#00f3ff"
              : isUnused
                ? "#475569" // Slate-600 (lighter gray) for unused
                : p.successRate > 0.8
                  ? "#0aff0a"
                  : p.successRate > 0.5
                    ? "#ffaa00"
            ...
            return (
              <g key={p.provider} className="transition-all duration-500">
                ...
                {/* Node (Logo) */}
                <g
                  transform={`translate(${x - (isActive ? 12 : 10)}, ${y - (isActive ? 12 : 10)})`}
                >
                  {getProviderLogo(
                    p.provider,
                    `w-${isActive ? 6 : 5} h-${isActive ? 6 : 5} ${isActive ? "text-white filter drop-shadow-[0_0_10px_rgba(0,243,255,0.8)]" : "text-gray-400"}`,
                  )}
                </g>
