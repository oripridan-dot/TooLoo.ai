...
export class Executive {
  ...
  private setupListeners() {
    ...
    this.bus.on('motor:result', async (event: SynapsysEvent) => {
      ...
      if (currentStep.status === 'running') {
        ...
        console.log(`[Executive] Reflection Result: ${reflection.action} - ${reflection.critique}`);
        ...
        if (reflection.action === 'CONTINUE') {
          ...
          if (reflection.modifiedStep) {
            ...
            this.currentPlan.steps[this.currentStepIndex] = reflection.modifiedStep;
            console.log(`[Executive] Applied modification to step payload.`);
          }
          // Re-execute the current step (index hasn't changed)
          const stepToRetry = this.currentPlan.steps[this.currentStepIndex];
          if (stepToRetry) {
            this.executeStep(stepToRetry);
          }
  ...
  public async execute(plan: Plan) {
    ...
    console.log(`[Executive] Starting execution of plan: ${plan.id} (${plan.steps.length} steps)`);
  ...
  private executeNextStep() {
    ...
    if (this.interventionMode) {
      this.isPaused = true;
      console.log(`[Executive] Pausing for intervention before step: ${step.description}`);
  ...
  private completePlan() {
    if (!this.currentPlan) return;
    console.log(`[Executive] Plan ${this.currentPlan.id} completed successfully.`);
