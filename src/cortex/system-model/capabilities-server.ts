  ...
  async loadPlugin(filePath) {
    try {
      ...
      if (PluginClass && typeof PluginClass === 'function') {
        ...
        const meta = plugin.metadata || { name: path.basename(filePath), version: '0.0.0' };
        ...
        this.plugins.set(meta.name, { instance: plugin, meta, status: 'active' });
...
app.post('/api/v1/events/publish', (req, res) => {
  ...
  if (!event) return res.status(400).json({ ok: false, error: 'Event name required' });
