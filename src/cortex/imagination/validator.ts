...
export class VisualValidator {
  ...
  private validateDiagram(mermaidCode: string): ValidationResult {
    ...
    const startsWithValid = validStarts.some((start) =>
      mermaidCode.trim().startsWith(start),
    );

    if (!startsWithValid) {
      issues.push(
        "Diagram does not start with a valid Mermaid keyword (graph, flowchart, etc.)",
      );
    }
    ...
    return {
      ...
      suggestion:
        issues.length > 0
          ? "Ensure the diagram starts with 'graph TD' or similar."
          : undefined,
    };
  }
  ...
  private validateComponent(componentJson: string): ValidationResult {
    ...
    try {
      ...
      if (JSON.stringify(component.props).includes("dangerouslySetInnerHTML")) {
        issues.push(
          "Component uses dangerouslySetInnerHTML which is flagged for review.",
        );
      }
    } catch (e) {
