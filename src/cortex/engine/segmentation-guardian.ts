...
export default class SegmentationGuardian {
  constructor({
    unifier,
    healthLogPath = path.join(
      process.cwd(),
      "quality",
      "segmentation-health.jsonl",
    ),
    statePath = path.join(
      process.cwd(),
      "quality",
      "segmentation-guardian-state.json",
    ),
    alertManager = null,
    thresholds = {
      p95LatencyMs: 200,
  ...
  readRecent(n = 300) {
    try {
      ...
      const lines = fs
        .readFileSync(this.healthLogPath, "utf8")
        .trim()
        .split("\n");
      return lines
        .slice(-n)
  ...
  async monitorAndAdjust() {
    ...
    let action = null;
    if (
      s.p95LatencyMs > this.thresholds.p95LatencyMs ||
      s.avgSegments > this.thresholds.avgSegmentsMax
    ) {
      // First step: reduce canary; if already very low, force rule mode
      const current = this.unifier.config;
      if (
        (current.mode === "hybrid" || current.mode === "ml") &&
        (current.canaryRatio ?? 0.1) > 0.02
      ) {
        const next = Math.max(
          0.01,
          Math.round(current.canaryRatio * 0.5 * 100) / 100,
        );
        this.unifier.saveConfig({ canaryRatio: next, mode: "hybrid" });
        action = { type: "reduce-canary", newCanary: next };
        if (this.alerts)
          this.alerts.alert(
            "segmentation.guardian.reduce_canary",
            `Reduced canary to ${next}`,
            { status: s },
          );
      } else if (current.mode !== "rule") {
        this.unifier.saveConfig({
          mode: "rule",
          rule: { variant: "advanced" },
        });
        action = { type: "switch-rule" };
        if (this.alerts)
          this.alerts.alert(
            "segmentation.guardian.switch_rule",
            "Switched to rule-only advanced due to drift",
            { status: s },
          );
      }
    }
  ...
  async gateConfigChange({
    baseUrl,
    suites = ["basic-segmentation"],
    minAccuracy,
  }) {
    // Run benchmark to validate change
    try {
      const { default: BenchmarkRunner } = await import(
        path.join(process.cwd(), "datasets/benchmarks/runner.js")
      );
      ...
      const passed =
        acc >= (minAccuracy ?? this.thresholds.minBenchmarkAccuracy);
      if (!passed && this.alerts)
        this.alerts.alert(
          "segmentation.guardian.gate_failed",
          `Benchmark failed at ${acc.toFixed(2)}% (min ${minAccuracy ?? this.thresholds.minBenchmarkAccuracy}%)`,
          { suites, baseUrl },
        );
  ...
  async runNightlyCycle({
    ...
    suites = ["basic-segmentation", "segmentation-100"],
    minAccuracy,
  } = {}) {
    const current = this.unifier.config;
    // Evaluate current config
    const evalRes = await this.gateConfigChange({
      baseUrl,
      suites,
      minAccuracy,
    });
    ...
    if (evalRes.passed) {
      ...
      if (
        (current.mode === "hybrid" || current.mode === "ml") &&
        (current.canaryRatio ?? 0.1) < 0.5
      ) {
        const bumped = Math.min(
          0.5,
          Math.round((current.canaryRatio ?? 0.1) * 1.25 * 100) / 100,
        );
        this.unifier.saveConfig({ mode: "hybrid", canaryRatio: bumped });
        actions.push({ type: "increase-canary", newCanary: bumped });
        if (this.alerts)
          this.alerts.alert(
            "segmentation.guardian.increase_canary",
            `Increased canary to ${bumped}`,
      ...
      if (lastGood?.config) {
        ...
        actions.push({
          type: "rollback",
          to: lastGood.config,
          accuracyWas: evalRes.accuracy,
        });
        if (this.alerts)
          this.alerts.alert(
            "segmentation.guardian.rollback",
            "Rolled back to last known good config",
            { accuracy: evalRes.accuracy, lastGood },
          );
      } else {
        const safe = {
          mode: "rule",
        ...
        if (this.alerts)
          this.alerts.alert(
            "segmentation.guardian.fallback",
            "Fallback to safe rule-only config",
            { accuracy: evalRes.accuracy },
          );
      }
    }
  ...
  noteGoodConfig(config, accuracy) {
    this.state.lastGood = { at: new Date().toISOString(), config, accuracy };
    this.saveState();
  }
  ...
  getStatus() {
    return {
      ...
      nightlyActive: !!this.nightlyTimer,
      state: this.state,
    };
  }
}
