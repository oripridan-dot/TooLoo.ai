...
export default class SegmentationUnifier {
  constructor({ strategies = {}, configPath } = {}) {
    ...
    this.configPath =
      configPath ||
      path.join(process.cwd(), "engine", "segmentation-config.json");
    this.healthLogPath = path.join(
      process.cwd(),
      "quality",
  ...
  async segment(messages, { strategy, context, shadowMl = true } = {}) {
    ...
    if (
      ...
      shadowMl &&
      Math.random() < (this.config.canaryRatio || 0)
    ) {
      const ml =
        this.strategies.get("ml-heuristic") || this.strategies.get("ml");
      if (ml) {
        Promise.resolve()
          .then(async () => {
  ...
  chooseStrategy(messages, override) {
    ...
    if (mode === "rule") {
      return this.config.rule?.variant === "advanced"
        ? "rule-advanced"
        : "rule-basic";
    }
    if (mode === "ml")
      return this.strategies.has("ml-heuristic") ? "ml-heuristic" : "ml";
    ...
    return this.strategies.has("ml-heuristic")
      ? "ml-heuristic"
      : "rule-advanced";
  }
  ...
  enforceContract(segments, messages) {
    ...
    for (const s of segments || []) {
      ...
      const summary = String(s.summary || messages[start]?.content || "").slice(
        0,
        200,
      );
