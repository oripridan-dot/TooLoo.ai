...
const OPTIMIZED_BASELINES = {
  ...
  Specialist: { concentration: 0.75, roi: 1.68, weight: 0.2 },
  "Power User": { velocity: 0.7, concentration: 0.6, roi: 1.47, weight: 0.18 },
...
export async function extractUserTraitsOptimized(userId, conversations) {
  ...
  const interactionFrequency = Math.min(
    conversations.length / Math.max(timeSpan, 1) / 10,
    1.0,
  );
...
export function traitDistanceOptimized(traits1, traits2) {
  const weights = {
    learningVelocity: 0.3, // Up from 0.25 - velocity is critical
...
export function clusterUsersByTraitsOptimized(userTraitsMap, k = null) {
  ...
  const centroids = [];
  centroids.push(
    userTraitsMap.get(userIds[Math.floor(Math.random() * userIds.length)]),
  );
  ...
  for (let i = 1; i < k; i++) {
    ...
    for (const userId of userIds) {
      ...
      for (const centroid of centroids) {
        minDistanceToCentroid = Math.min(
          minDistanceToCentroid,
          traitDistanceOptimized(traits, centroid),
        );
      }
...
export function generateOptimizedCohortMetadata(cluster, cohortId, idx) {
  ...
  if (archetype === "Fast Learner") {
    const velocityBonus =
      cluster[0][1].velocityTrend === "improving" ? 1.05 : 1.0;
    roiMultiplier =
      roiVelocity(avgTraits.learningVelocity, {
        r_squared: 0.6,
        slope: 0.001,
      }) * velocityBonus;
    ...
    roiMultiplier = calculateRetentionROI(
      avgTraits.retentionStrength,
      cluster[0][1].engagement?.trajectory || "stable",
    );
  }
  ...
  return {
    ...
    roi: {
      ...
      optimized: Math.round(roiMultiplier * 1000) / 1000,
      improvement:
        Math.round(((roiMultiplier - baseline.roi) / baseline.roi) * 100) + "%",
    },
...
export async function discoverCohortsOptimized(userConversationMap) {
  ...
  for (let i = 0; i < clusters.length; i++) {
    const metadata = generateOptimizedCohortMetadata(
      clusters[i],
      `cohort-optimized-${i}`,
      i,
    );
...
export async function saveOptimizedCohorts(cohorts, filepath = null) {
  if (!filepath) {
    filepath = path.join(
      __dirname,
      "../data/segmentation/cohorts-optimized.json",
    );
  }
...
async function main() {
  console.log(
    "🚀 Phase 3 Sprint 2 - Task 4: Integration & Recalibration (Optimized)",
  );
  ...
  result.cohorts.forEach((cohort, idx) => {
    ...
    console.log(
      `  ROI: ${cohort.roi.baseline}x → ${cohort.roi.optimized}x (${cohort.roi.improvement})`,
    );
  ...
  const portfolioRoiAfter =
    result.cohorts.reduce((sum, c) => sum + parseFloat(c.roi.optimized), 0) /
    result.cohorts.length;
  const improvement = (
    ((portfolioRoiAfter - portfolioRoiBefore) / portfolioRoiBefore) *
    100
  ).toFixed(1);
