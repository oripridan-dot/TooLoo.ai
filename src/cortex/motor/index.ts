...
export class MotorCortex {
  ...
  private setupListeners() {
    ...
    this.bus.on('motor:code:execute', async (event: SynapsysEvent) => {
      ...
      console.log(`[MotorCortex] Received code execution request (${payload.language})`);

      const result = await this.codeExecutor.executeInDocker(payload.code, payload.language);
    ...
    this.bus.on('motor:execute', async (event: SynapsysEvent) => {
      const payload = event.payload;
      console.log(`[MotorCortex] Received execution request: ${payload.command}`);

      const result = await this.executor.runCommand(payload.command, payload.cwd);
