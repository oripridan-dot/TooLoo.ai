...
export class ProviderFeedbackEngine {
  ...
  /**
   * Check if provider should be tracked (starts with 'non' prefix)
   */
  private shouldTrack(providerName: string): boolean {
   ...
   */
  recordRequestStart(providerName: string, requestId?: string) {
  ...
  recordRequestSuccess(
    ...
    feedback.avgLatencyMs = Math.round(
      feedback.totalLatencyMs / feedback.successCount,
    );
   ...
   */
  recordRequestError(providerName: string, errorMessage?: string) {
    if (!this.shouldTrack(providerName)) return;

    const feedback = this.getOrCreateProvider(providerName);
    feedback.status = "error";
    feedback.successRate = feedback.successCount / feedback.callCount;
    feedback.confidenceScore = Math.max(
      0,
      feedback.confidenceScore - 0.1,
    );
    feedback.currentRequest = undefined;
