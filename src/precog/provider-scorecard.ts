...
export class ProviderScorecard {
    private stats: Map<string, { requests: number, totalLatency: number, successful: number }> = new Map();
    ...
    recordMetric(provider: string, latencyMs: number, success: boolean) {
        const current = this.stats.get(provider) || { requests: 0, totalLatency: 0, successful: 0 };
