...
export default class TrainingCamp {
  ...
  private backgroundEveryNRounds: number = 5;
  private _backgroundCursor: number = 0;
      ...
      dsa: {
        ...
        problems: [
          ...
          },
          {
            ...
            keywords: ['inorder', 'traversal', 'sorted'],
          },
          {
            id: 'dsa_003',
            topic: 'DP',
            ...
            keywords: ['dynamic', 'programming', 'O(n log n)'],
          },
        ],
      },
      os: {
        ...
        problems: [
          {
            ...
            topic: 'Scheduling',
            difficulty: 'easy',
            problem: 'Calculate FCFS waiting time',
            keywords: ['waiting time', 'arrival', 'average'],
          },
          {
            id: 'os_002',
            topic: 'Sync',
            difficulty: 'medium',
            problem: 'Detect deadlock in resource allocation',
            keywords: ['circular wait', 'deadlock', 'resource'],
          },
          {
            id: 'os_003',
            topic: 'Memory',
            ...
            keywords: ['page replacement', 'optimal', 'future'],
          },
        ],
      },
      networks: {
        ...
        problems: [
          {
            ...
            topic: 'TCP/IP',
            difficulty: 'easy',
            problem: 'Calculate subnet and broadcast address',
            keywords: ['subnet', 'network', 'broadcast', 'CIDR'],
          },
          {
            id: 'net_002',
            topic: 'HTTP',
            difficulty: 'medium',
            problem: 'Explain GET vs POST methods',
            keywords: ['GET', 'POST', 'idempotent'],
          },
          {
            id: 'net_003',
            topic: 'DNS',
            difficulty: 'hard',
            problem: 'Trace DNS resolution from root',
            keywords: ['DNS', 'root', 'TLD', 'authoritative'],
          },
        ],
      },
      compilers: {
        ...
        problems: [
          {
            ...
            topic: 'Lexer',
            difficulty: 'easy',
            problem: 'Role of lexer in compiler',
            keywords: ['lexer', 'tokens', 'source'],
          },
          {
            id: 'comp_002',
            topic: 'Parsing',
            difficulty: 'medium',
            problem: 'Top-down vs bottom-up parsing',
            keywords: ['top-down', 'bottom-up', 'grammar'],
          },
          {
            id: 'comp_003',
            topic: 'Optimization',
            difficulty: 'hard',
            problem: 'Loop unrolling benefits',
            keywords: ['loop unrolling', 'optimization', 'performance'],
          },
        ],
      },
      databases: {
        ...
        problems: [
          {
            ...
            topic: 'SQL',
            difficulty: 'easy',
            problem: 'SELECT all users from table',
          ...
          },
          {
            ...
            topic: 'Transactions',
            difficulty: 'medium',
            problem: 'Explain ACID properties',
          ...
          },
          {
            ...
            topic: 'Indexing',
            difficulty: 'hard',
            problem: 'How B-tree index works',
            keywords: ['B-tree', 'index', 'lookup'],
          },
        ],
      },
      ml: {
        ...
        problems: [
          {
            ...
            topic: 'Supervised',
            difficulty: 'easy',
            problem: 'Define supervised learning',
            keywords: ['supervised', 'labeled', 'classification'],
          },
          {
            id: 'ml_002',
            topic: 'Overfitting',
            difficulty: 'medium',
            problem: 'What is overfitting, how to prevent',
          ...
          },
          {
            ...
            topic: 'Neural Nets',
            difficulty: 'hard',
            problem: 'Role of activation functions',
            keywords: ['activation', 'non-linearity', 'neural'],
          },
        ],
      },
      security: {
        ...
        problems: [
          {
            ...
            topic: 'Auth',
            difficulty: 'easy',
            problem: 'Authentication vs Authorization',
          ...
          },
          {
            ...
            keywords: ['SQL injection', 'parameterized', 'validation'],
          },
          {
            id: 'sec_003',
            topic: 'Crypto',
            difficulty: 'hard',
            problem: 'Symmetric vs asymmetric encryption',
        ...
        problems: [
          {
            ...
            topic: 'Complexity',
            difficulty: 'easy',
            problem: 'Define P and NP classes',
            keywords: ['P', 'NP', 'polynomial'],
          },
          {
            id: 'theory_002',
            topic: 'Automata',
            difficulty: 'medium',
            problem: 'What is finite automaton',
            keywords: ['finite automaton', 'regular language', 'state'],
          },
          {
            id: 'theory_003',
            topic: 'NP-Complete',
            difficulty: 'hard',
            problem: 'What makes problem NP-complete',
            keywords: ['NP-complete', 'reduction', 'polynomial'],
          },
        ],
      },
      distributed: {
        ...
        problems: [
          ...
          },
          {
            ...
            topic: 'Consensus',
            difficulty: 'medium',
            problem: 'Raft consensus algorithm',
            keywords: ['Raft', 'consensus', 'leader', 'replication'],
          },
          {
            id: 'dist_003',
            topic: 'Consistency',
            difficulty: 'hard',
            problem: 'Eventual consistency use cases',
            keywords: ['eventual consistency', 'convergence', 'replicas'],
          },
        ],
      },
    };
  ...
  gradeProblem(topic: string, problemId: string, response: string) {
    const curriculumTopic = this.curriculum[topic];
    if (!curriculumTopic) {
      return { score: 0, feedback: 'Topic not found', maxScore: 100 };
    }
    const problem = curriculumTopic.problems.find((p) => p.id === problemId);
  ...
  selectWeakestDomains(count = 3) {
    ...
    if (this.selection?.autoFillGaps) {
      ...
      if (
        !this.selection.stickyBatch ||
        batch.length < Math.min(count, Number(this.selection.gapsCount || count))
      ) {
        ...
        const belowSorted = this.topics
          ...
          .map((t) => ({
            topic: t,
            mastery: this.progress[t]?.masteryLevel || 0,
            confidence: this.progress[t]?.confidence || 50,
          }))
          .filter((d) => !this.isBackground(d.topic))
          .filter((d) => d.mastery < threshold)
          .sort((a, b) =>
            a.mastery !== b.mastery ? a.mastery - b.mastery : a.confidence - b.confidence
          )
          .map((d) => d.topic);
        const need = Math.min(count, Number(this.selection.gapsCount || count));
        for (const t of belowSorted) {
    ...
    return Array.from(picks) as string[];
  }

  // Check if domain needs spaced repetition review
  needsReview(topic: string) {
  ...
  scheduleNextReview(topic: string, intervalIndex = 0) {
    const interval =
      this.spacedRepetition.intervals[intervalIndex] ||
      this.spacedRepetition.intervals[this.spacedRepetition.intervals.length - 1];
    this.spacedRepetition.nextReviews[topic] = Date.now() + (interval || 1800000);
  }
  ...
  async startCamp() {
    ...
    console.log(
      `📚 Training across ${this.topics.length} domains: ${this.topics.map((t) => this.curriculum[t]?.name || t).join(', ')}`
    );
    console.log(
      `⚡ Parallel training: ${this.maxParallel} domains at once (cpu cores: ${os.cpus()?.length || 'n/a'})`
    );
    ...
    try {
      await this.preGenerateVariations();
    } catch {
      /* ignore */
    }
  ...
  async preGenerateVariations() {
    ...
    for (const topic of this.topics) {
      ...
      out[topic] = problems.map((p: any, idx) => ({
        id: p.id,
        topic: p['topic'],
        ...
        variants: [
          {
            ...
            promptHint: `${p['problem']} (v1)`,
          },
          {
            seed: 2 + idx,
            difficulty: p['difficulty'],
            promptHint: `${p['problem']} (v2)`,
          },
        ],
      }));
    }
  ...
  async runRound() {
    if (!this.isActive) {
      return {
        success: false,
        error: 'Training camp not started. Call startCamp() first.',
      };
    }
    ...
    for (const topic of activeDomains) {
      ...
      if (this.spacedRepetition.enabled) {
        const topicProgress = topic ? this.progress[topic] : undefined;
        if (topicProgress) {
          const reviewIndex = Math.min(
            topicProgress.attempts - 1,
            this.spacedRepetition.intervals.length - 1
          );
    ...
    try {
      if (
        this.backgroundTopics.length > 0 &&
        this.backgroundEveryNRounds > 0 &&
        this.currentRound % this.backgroundEveryNRounds === 0
      ) {
        const bgTopic =
          this.backgroundTopics[this._backgroundCursor % this.backgroundTopics.length];
    ...
    if (this.spacedRepetition.enabled) {
      for (const topic of this.topics) {
        const topicProgress = this.progress[topic];
        if (
          !activeDomains.includes(topic) &&
          this.needsReview(topic) &&
          topicProgress &&
          ...
          this.scheduleNextReview(
            topic,
            Math.min(topicProgress.attempts, this.spacedRepetition.intervals.length - 1)
          );
        }
      }
    }
  ...
  async runTopicProblems(topic: string, count = 1) {
    const curriculumTopic = this.curriculum[topic];
    if (!curriculumTopic) {
      return { topic, topicName: 'Unknown', problemsSolved: 0, avgScore: 0, results: [] };
    }
    const problems = curriculumTopic.problems;
    const results: Array<{
      problemId: string;
      problem: string;
      score: number;
    ...
    for (let i = 0; i < Math.min(count, problems.length); i++) {
      ...
      if (progress) {
        ...
        (progress as any).lastPracticed = new Date();
  ...
  async solveProblem(topic: string, problem: Problem) {
    // Simulate solving problem by generating response with keywords
    const responseKeywords = problem.keywords.slice(
      0,
      Math.floor(Math.random() * problem.keywords.length) + 1
    );
    ...
    return {
      ...
      problem: (problem as any).problem || '',
      score: gradeResult.score,
      feedback: gradeResult.feedback,
      difficulty: (problem as any).difficulty || 'medium',
    };
  }

  getMasteryScore(topic: string) {
    const progress = this.progress[topic];
  ...
  updateMasteryScores() {
    for (const topic of this.topics) {
      ...
      progress.confidence = Math.min(
        95,
        Math.max(30, progress.masteryLevel * 0.7 + consistency * 0.3)
      );
    }
  }
  ...
  getProgressSummary() {
    ...
    for (const topic of this.topics) {
      ...
      summary[topic] = {
        name: this.curriculum[topic]?.name || topic,
        ...
        trend:
          progress.scores.length >= 2
            ? progress.scores.slice(-1)[0]! > progress.scores.slice(-2)[0]!
              ? 'improving'
              : 'stable'
            : 'initial',
   ...
   */
  calibrateToThreshold(
    options: { threshold?: number; minAttempts?: number; delta?: number; force?: boolean } = {}
  ) {
  ...
  peekNextTargets(count = 3) {
    const threshold = Number(this.selection?.targetThreshold || 80);
    if (this.selection?.autoFillGaps) {
      const locked = (this._lockedBatch || []).filter(
        (t) => (this.progress[t]?.masteryLevel || 0) < threshold
      );
      ...
      const belowSorted = this.topics
        ...
        .map((d) => d.topic);
      for (const t of belowSorted) {
        if (batch.length >= Math.min(count, Number(this.selection.gapsCount || count))) break;
        batch.push(t);
      }
      if (batch.length > 0)
        return batch.slice(0, Math.min(count, Number(this.selection.gapsCount || count)));
      // else fall through
    }
    ...
    const picks: string[] = [];
    if (this.selection?.targetedMode) {
      const untouched = this.topics.filter(
        (t) => (this.progress[t]?.attempts || 0) === 0 && !this.isBackground(t)
      );
  ...
  setSettings(
    ...
    if (typeof partial.parallelTraining === 'boolean')
      this.parallelTraining = partial.parallelTraining;
    if (typeof partial.maxParallel === 'number')
      this.maxParallel = Math.max(1, Math.min(9, partial.maxParallel));
    if (typeof partial.spacedRepetition === 'boolean')
      this.spacedRepetition.enabled = partial.spacedRepetition;
    if (partial.selection && typeof partial.selection === 'object') {
      this.selection = {
        ...this.selection,
        ...partial.selection,
        targetThreshold: Number(
          partial.selection.targetThreshold ?? this.selection.targetThreshold
        ),
        autoFillGaps:
          typeof partial.selection.autoFillGaps === 'boolean'
            ? partial.selection.autoFillGaps
            : this.selection.autoFillGaps,
        gapsCount: Number(partial.selection.gapsCount ?? this.selection.gapsCount),
        stickyBatch:
          typeof partial.selection.stickyBatch === 'boolean'
            ? partial.selection.stickyBatch
            : (this.selection.stickyBatch ?? false),
        minAttemptsInBatch: Number(
          partial.selection.minAttemptsInBatch ?? this.selection.minAttemptsInBatch
        ),
      };
      // Reset locked batch if autoFillGaps toggled off or threshold/gapsCount changed significantly
      if (!this.selection.autoFillGaps) {
        this._lockedBatch = [];
        this._batchJoinAttempts = {};
      }
    }
  ...
  getActiveTrainingData() {
    ...
    const activeData = weakest.map((topic) => {
      ...
      return {
        ...
        uncertaintyBand: Math.round(100 - progress.confidence),
        attempts: progress.attempts,
        problems: problems.map((p: any, idx) => ({
          id: p.id,
          topic: p['topic'],
          difficulty: p['difficulty'],
  ...
  getDeepDiveData(topic: string): DeepDiveData {
    ...
    if (!curriculum) {
      return { error: 'Topic not found in curriculum' };
    }
    const nextReview = this.spacedRepetition.nextReviews[topic];
    ...
    return {
      ...
      problems: curriculum.problems.map((p: any, idx) => ({
        id: p.id,
        topic: p['topic'],
        difficulty: p['difficulty'],
        ...
        attemptsOnThis:
          Math.floor(progress.attempts / curriculum.problems.length) +
          (idx < progress.attempts % curriculum.problems.length ? 1 : 0),
      })),
  ...
  addTopic(topic: Topic) {
    ...
    if (this.topics.includes(topic.key)) {
      ...
      this.curriculum[topic.key] = {
        name: topic.name || this.curriculum[topic.key]?.name || topic.key,
        problems: Array.isArray(topic.problems)
          ? (topic.problems as Problem[])
          : this.curriculum[topic.key]?.problems || [],
      };
      if (topic.background && !this.backgroundTopics.includes(topic.key))
        this.backgroundTopics.push(topic.key);
      return {
        ok: true,
        key: topic.key,
    ...
    this.curriculum[topic.key] = {
      name: topic.name || topic.key,
      problems: Array.isArray(topic.problems) ? (topic.problems as Problem[]) : [],
    };
