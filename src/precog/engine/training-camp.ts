...
export default class TrainingCamp {
  ...
  isBackground(topic: string) {
    return (
      Array.isArray(this.backgroundTopics) &&
      this.backgroundTopics.includes(topic)
    );
  }
  ...
  gradeProblem(topic: string, problemId: string, response: string) {
    const problem = this.curriculum[topic].problems.find(
      (p) => p.id === problemId,
    );
    if (!problem)
      return { score: 0, feedback: "Problem not found", maxScore: 100 };
    ...
    for (const keyword of problem.keywords) {
      ...
      if (
        ...
        (synonyms[keyLower] &&
          synonyms[keyLower].some((syn: string) => responseText.includes(syn)))
      ) {
    ...
    const baseScore = Math.round(
      (matchedKeywords.length / problem.keywords.length) * 100,
    );
  ...
  selectWeakestDomains(count = 3) {
    ...
    if (this.selection?.autoFillGaps) {
      ...
      if (
        ...
        batch.length <
          Math.min(count, Number(this.selection.gapsCount || count))
      ) {
      ...
      if (this._lockedBatch.length > 0)
        return this._lockedBatch.slice(0, count);
    ...
    if (this.selection?.targetedMode) {
      const untouched = this.topics.filter(
        (t) => (this.progress[t]?.attempts || 0) === 0,
      );
      if (untouched.length > 0) {
        if (this.selection.roundRobinZeros) {
          ...
          this._untouchedCursor =
            (this._untouchedCursor + count) % Math.max(1, untouched.length);
        } else {
  ...
  scheduleNextReview(topic, intervalIndex = 0) {
    const interval =
      ...
      this.spacedRepetition.intervals[
  ...
  async startCamp() {
    ...
    console.log(
      `📚 Training across ${this.topics.length} domains: ${this.topics.map((t) => this.curriculum[t].name).join(", ")}`,
    );
    console.log(
      `⚡ Parallel training: ${this.maxParallel} domains at once (cpu cores: ${os.cpus()?.length || "n/a"})`,
    );
    console.log(
      `🔄 Spaced repetition: ${this.spacedRepetition.enabled ? "ON" : "OFF"}`,
    );
  ...
  async runRound() {
    ...
    for (const topic of activeDomains) {
      ...
      if (this.spacedRepetition.enabled) {
        const reviewIndex = Math.min(
          this.progress[topic].attempts - 1,
          this.spacedRepetition.intervals.length - 1,
        );
    ...
    try {
      if (
        ...
        const bgTopic =
          this.backgroundTopics[
            this._backgroundCursor % this.backgroundTopics.length
          ];
        this._backgroundCursor =
          (this._backgroundCursor + 1) % this.backgroundTopics.length;
    ...
    if (this.spacedRepetition.enabled) {
      for (const topic of this.topics) {
        if (
          ...
          this.progress[topic].attempts > 0
        ) {
  ...
  async solveProblem(topic, problem) {
    ...
    const simulatedResponse =
      responseKeywords.join(" ") + " additional context";
  ...
  calibrateToThreshold(options = {}) {
    const threshold = Number(
      options.threshold || this.selection?.targetThreshold || 80,
    );
    const minAttempts = Number(
      options.minAttempts || this.selection?.minAttemptsInBatch || 2,
    );
  ...
  setSettings(partial = {}) {
    ...
    if (partial.selection && typeof partial.selection === "object") {
      this.selection = {
        ...
        gapsCount: Number(
          partial.selection.gapsCount ?? this.selection.gapsCount,
        ),
        stickyBatch:
          typeof partial.selection.stickyBatch === "boolean"
            ...
            : this.selection.stickyBatch,
        minAttemptsInBatch: Number(
          partial.selection.minAttemptsInBatch ??
            this.selection.minAttemptsInBatch,
        ),
      };
  ...
  addTopic(topic: Topic) {
    if (!topic.key || typeof topic.key !== "string")
