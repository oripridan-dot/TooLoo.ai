...
export class ValidatedExecutionFramework {
  ...
  async safeExecute(fn, params = {}, metadata = {}) {
    ...
    try {
      ...
      const preValidation = await this.validatePreExecution(
        fn,
        params,
        metadata,
      );
  ...
  async validateResult(result, metadata) {
    ...
    if (metadata.expectedResult) {
      if (
        metadata.expectedResult.type &&
        typeof result !== metadata.expectedResult.type
      ) {
        checks.push({
          name: "result_type",
          passed: false,
          error: `Expected ${metadata.expectedResult.type}, got ${typeof result}`,
        });
        passed = false;
      } else {
