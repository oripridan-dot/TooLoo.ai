...
interface MetaLearningOptions {
  ...
  selfDiscoveryEngine?: any;
  enhancedLearning?: any;
  autonomousEvolutionEngine?: any;
  userModelEngine?: any;
  contextBridgeEngine?: any;
  predictiveEngine?: any;
  proactiveIntelligenceEngine?: any;
}
...
export default class MetaLearningEngine {
  ...
  private selfDiscovery: any;
  private enhancedLearning: any;
  private autonomousEvolution: any;
  private userModelEngine: any;
  private contextBridgeEngine: any;
  private predictiveEngine: any;
  private proactiveIntelligenceEngine: any;
  private started: boolean;
  private phases: Record<number, PhaseInfo>;
  private metrics: MetricsState;
  private perMetricStrategies: PerMetricStrategies;
  ...
  getNextPhaseNumber(): number | null {
    for (let i = 1; i <= 4; i++) {
      const phase = this.phases[i];
      if (phase && phase.status !== 'completed') return i;
    }
    return null;
  }
  ...
  async runPhase(phaseNumber: number) {
    ...
    try {
      ...
      this.log(`Phase ${phaseNumber} failed`, { error: e?.message || String(e) });
      await this.saveState();
      throw e;
    }
  }
  ...
  async phase1_LearningAnalysis() {
    ...
    if (realMetrics) {
      ...
      if (this.selfDiscovery) {
        ...
        const hidden = recent.flatMap((d: any) => d?.details?.hiddenCapabilities || []);
        const learningTargets = hidden.filter((h: any) =>
    ...
    const phase1 = this.phases[1];
    if (phase1) phase1.findings = findings;
  ...
  async phase2_MetaLearningAlgorithms() {
    ...
    const proposals = [
      {
        id: 'meta_loop_1',
        description: 'Meta-cognitive loop to evaluate and select learning strategies per task',
      },
      {
        id: 'meta_opt_1',
        description: 'Optimize learning rate dynamically based on feedback and error signals',
      },
      {
        id: 'meta_mem_1',
        description: 'Memory consolidation routine to improve retention over time',
      },
    ];
    ...
    const phase2 = this.phases[2];
    if (phase2) phase2.changes = changes;
  ...
  async phase3_AdaptiveLearningStrategies() {
    ...
    const phase3 = this.phases[3];
    if (phase3) phase3.strategies = strategies;
    this.metrics.improvements.push({
      phase: 3,
  ...
  async phase4_LearningAcceleration() {
    ...
    const phase4 = this.phases[4];
    if (phase4) phase4.results = results;
    this.metrics.improvements.push({
      phase: 4,
      metric: 'acceleration',
  ...
  async boostRetention({ retentionDelta = 0.05, transferDelta = 0.04 } = {}) {
    const kr = Math.max(
      0,
      Math.min(1, (this.metrics.current.knowledgeRetention || 0) + Number(retentionDelta || 0))
    );
    const te = Math.max(
      0,
      Math.min(1, (this.metrics.current.transferEfficiency || 0) + Number(transferDelta || 0))
    );
    const before = {
  ...
  analyzeTrajectory(windowSize = 15) {
    ...
    const meanY = sumY / n;
    const ssRes = y.reduce((sum, yi, i) => sum + Math.pow(yi - (m * (x[i] ?? 0) + b), 2), 0);
  ...
  calculateAcceleration(deltas: number[]): number {
    if (deltas.length < 3) return 0;

    const differences: number[] = [];
    for (let i = 1; i < deltas.length; i++) {
      differences.push((deltas[i] ?? 0) - (deltas[i - 1] ?? 0));
    }
  ...
  calculateTrajectoryConfidence(slope: number, r2: number, acceleration: number): number {
  ...
  forecastMetrics(cycles = 3) {
    ...
    const recentAvg = trajectory.recentAvg ?? 0;
  ...
  predictPlateauTiming(confidenceThreshold = 0.65) {
    ...
    const plateauForecast = forecastData.find((f: any) => f.willPlateau);
    ...
    const confidence = trajectory.confidence ?? 0;
    ...
    return {
      ...
      reason: `Trajectory analysis (R²=${(trajectory.r2 ?? 0).toFixed(2)}, slope=${(trajectory.slope ?? 0).toFixed(4)}) predicts plateau in ${plateauForecast.cycle} cycles`,
      forecast: forecastData,
      recommendedAction:
        confidence >= confidenceThreshold ? 'trigger_preemptive_strategy' : 'monitor',
    };
  }
  ...
  async applyPerMetricStrategies(confidenceThreshold = 0.7) {
    ...
    const getMetricTrend = (metricFilter: (i: any) => boolean) => {
    ...
    const trends = {
      learningVelocity: getMetricTrend((i) => i.metric && i.metric.includes('learning')),
      adaptationSpeed: getMetricTrend((i) => i.metric && i.metric.includes('adaptation')),
      knowledgeRetention: getMetricTrend((i) => i.metric && i.metric.includes('retention')),
      transferEfficiency: getMetricTrend((i) => i.metric && i.metric.includes('transfer')),
    };

    // Apply per-metric strategies
    const strategyResults = this.perMetricStrategies.applyStrategies(
      this.metrics.current as unknown as Record<string, number>,
      trends
    );
    ...
    const result: any = {
      ...
      afterMetrics: {} as Record<string, any>,
    };

    // Update metrics with strategy boosts
    for (const [metric, stratResult] of Object.entries(strategyResults) as [string, any][]) {
      (this.metrics.current as any)[metric] = stratResult.after;
  ...
  async fetchRealMetricsFromTraining() {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${trainingPort}/api/v1/training/overview`);
      ...
      const avgMastery =
        domains.length > 0
          ? domains.reduce((sum: number, d: any) => sum + (d.mastery || 0), 0) / domains.length
          : 0.35;
      ...
      const realMetrics = {
        ...
        transferEfficiency: Math.max(0, Math.min(1.0, successRate * 0.9 || 0.35)),
      };
  ...
  async phase1_LearningAnalysisEnhanced() {
    ...
    const phase1Enhanced = this.phases[1];
    if (phase1Enhanced) phase1Enhanced.findings = findings;
  ...
  async checkPlateauPredictiveAndReactive(confidenceThreshold = 0.7) {
    ...
    if (prediction.plateauPredicted) {
      ...
      return {
        ...
        reason: `Predictive: Plateau forecast in ${prediction.cyclesUntilPlateau} cycle(s) (confidence: ${((prediction.confidence ?? 0) * 100).toFixed(1)}%)`,
      };
    }
  ...
  async checkPlateauAndAutoAdapt() {
    ...
    if (check.plateauDetected) {
      const checkConfidence = (check as any).confidence ?? 0;
      this.log('Auto-adapting due to plateau', {
        type: check.type,
        cyclesUntilPlateau: (check as any).cyclesUntilPlateau,
        usePerMetric: !!this.perMetricStrategies && checkConfidence > 0.7,
      });

      // Phase 2B: Use per-metric strategies if available and confident
      if (this.perMetricStrategies && checkConfidence > 0.7) {
        return await this.applyPerMetricStrategies(checkConfidence);
      } else {
