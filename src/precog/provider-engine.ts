...
export class ProviderEngine {
  ...
  async generateImage(
    ...
    console.log(
      `[ProviderEngine] Generating image with prompt: ${req.prompt.substring(0, 50)}...`,
    );

    if (
      req.provider === "openai" ||
      (req.model && req.model.includes("dall-e"))
    ) {
      try {
        const base64Image = await this.openaiImageProvider.generateImage(
          req.prompt,
          {
            model: req.model,
            size: req.imageSize,
  ...
  private classifyComplexity(
    ...
    if (taskType === "code_generation" || taskType === "architecting")
      return "high";
