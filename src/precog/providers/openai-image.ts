...
export class OpenAIImageProvider {
  ...
  async generateImage(
    ...
    console.log(
      `[OpenAIImageProvider] Generating image with model dall-e-3...`,
    );
  ...
  async generateCodeFromContext(
    ...
    console.log(
      `[OpenAIImageProvider] Generating code from context with DALL-E 3...`,
    );
    ...
    try {
      ...
      if (req.includeImage) {
        try {
          ...
          console.warn(
            "[OpenAIImageProvider] Failed to generate visual mockup:",
            e,
          );
        }
      }
