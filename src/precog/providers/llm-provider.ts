...
export default class LLMProvider {
  ...
  selectProvider(taskType = 'chat') {
    ...
    if (taskType === 'code' || taskType === 'programming') {
      const codeOrder = ['gemini', 'openinterpreter', 'ollama', 'deepseek', 'openai', 'anthropic'];
    ...
    if (taskType === 'reasoning' || taskType === 'analysis') {
      const reasoningOrder = ['gemini', 'anthropic', 'openai', 'deepseek', 'ollama'];
  ...
  async generateSmartLLM(request) {
    ...
    const order = ['gemini', 'ollama', 'anthropic', 'openai', 'localai', 'huggingface', 'openinterpreter', 'deepseek'];
...
const providerBuilders = {
  ...
  gemini: () => {
    ...
    const model = env('GEMINI_MODEL', 'gemini-3.0-pro-preview');
    return {
      name: 'gemini',
...
export async function generateSmartLLM({ prompt, system, taskType, criticality = 'normal', maxTokens }) {
  ...
  const criticalityRouting = {
    low: {
      creative: ['gemini','ollama','huggingface','localai','deepseek','openai','claude'],
      reasoning: ['gemini','ollama','huggingface','deepseek','claude','openai'], 
      general: ['gemini','ollama','huggingface','localai','deepseek','openai','claude']
    },
    normal: {
      creative: ['gemini','deepseek','ollama','huggingface','claude','openai'],
      reasoning: ['gemini','deepseek','claude','ollama','openai','huggingface'],
      general: ['gemini','deepseek','ollama','openai','claude','huggingface','localai']
    },
    high: {
