...
export class Harvester {
  ...
  constructor(providers: ProviderEngine) {
    ...
    this.refinery = new Refinery(providers);
    this.truthEngine = new TruthEngine(providers);
    this.setupListeners();
  ...
  async harvest(request: HarvestRequest): Promise<any> {
    console.log(
      `[Harvester] Processing request for: ${request.url} (${request.type})`,
    );
    ...
    const verification = await this.truthEngine.verify(
      refinedData.markdown || JSON.stringify(refinedData),
      `Source: ${request.url}`,
    );
    ...
    if (verification.isAccurate) {
      bus.publish("cortex", "memory:store", {
        content: refinedData.markdown || JSON.stringify(refinedData),
  ...
  private async collectMedia(request: HarvestRequest): Promise<HarvestResult> {
    return new Promise((resolve, reject) => {
      const scriptPath = path.join(
        process.cwd(),
        "src/precog/harvester/sensory/processor.py",
      );
      ...
      pythonProcess.on("close", (code) => {
        if (code !== 0) {
          reject(
            new Error(`Media processor failed (code ${code}): ${errorOutput}`),
          );
          return;
        }
        ...
        try {
          ...
          reject(
            new Error(`Failed to parse media processor output: ${e.message}`),
          );
        }
      });
    });
