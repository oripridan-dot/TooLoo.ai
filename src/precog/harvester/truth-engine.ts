...
export class TruthEngine {
    ...
    async verify(content: string, context: string = ''): Promise<VerificationResult> {
        return this.validator.safeExecute(async () => {
            ...
            const response = await precog.providers.generate({
                prompt: prompt,
                system: "You are a strict fact-checking engine. You do not hallucinate. You are skeptical by default.",
                taskType: 'analysis'
            });
            ...
            try {
                ...
                console.warn("[TruthEngine] Failed to parse verification response, defaulting to raw analysis.");
                return {
        ...
        }, { content, context }, { component: 'TruthEngine' });
    }
}
