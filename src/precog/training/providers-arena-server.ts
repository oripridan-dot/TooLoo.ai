...
environmentHub.registerComponent("providersArena", { version: "1.0" }, [
  "multi-provider",
...
app.post("/api/v1/arena/query", async (req, res) => {
  try {
    ...
    if (!providers || providers.length === 0) {
      return res
        .status(400)
        .json({ ok: false, error: "At least one provider required" });
    }
    ...
    if (validProviders.length === 0) {
      return res
        .status(400)
        .json({ ok: false, error: "No valid providers in request" });
    }
    ...
    if (failed.length > 0) {
      console.warn(
        `[Arena] ${failed.length} provider(s) failed:`,
...
app.post("/api/v1/arena/synthesize", async (req, res) => {
  try {
    ...
    if (Object.keys(responses).length < 2) {
      return res
        .status(400)
        .json({ ok: false, error: "Need at least 2 responses" });
    }
...
app.post("/api/v1/arena/consensus", async (req, res) => {
  try {
    ...
    const agreementLevel = Math.min(
      100,
      Math.round((commonThemesCount / responseCount) * 50 + 50),
    );
...
app.post("/api/v1/arena/compare", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ ok: false, error: "Two providers with responses required" });
    }

    const textA =
      typeof responses[providerA] === "string"
        ? responses[providerA]
        : responses[providerA].text;
    const textB =
      typeof responses[providerB] === "string"
        ? responses[providerB]
        : responses[providerB].text;
...
app.get("/api/v1/arena/status", (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      providers: status,
      available: Object.values(status).filter((s) => s.available && s.enabled)
        .length,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
...
app.post("/api/v1/arena/test", (req, res) => {
  ...
  res.status(503).json({
    ok: false,
    error:
      "Provider API keys not configured. Set CLAUDE_API_KEY, OPENAI_API_KEY, or other provider credentials in .env for real responses.",
    message:
      "Test endpoint requires real provider configuration - mocks removed for truthful system",
  });
});
...
app.post("/api/v1/arena/conflicts", async (req, res) => {
  try {
    ...
    try {
      ...
      conflicts =
        "- Some providers may have different perspectives on the topic\n- Evaluate each position on its merits";
    }
...
app.get("/health", (req, res) => {
  res.json({
    ok: true,
    service: "providers-arena-server",
...
const server = app.listen(PORT, "127.0.0.1", () => {
  ...
  console.log(
    "📊 Endpoints: /api/v1/arena/{query,synthesize,consensus,compare,tldr,questions,actions,conflicts,events,export,history,status}",
  );
});
server.on(
  "error",
  (err) =>
    console.error("[providers-arena-server] Error:", err.message) &&
    process.exit(1),
);
