...
const hyperCamp = new HyperSpeedTrainingCamp({
...
app.post("/api/v1/sources/github/issues/sync", async (req, res) => {
  try {
    ...
    if (!r.ok) {
      return res
        .status(r.status)
        .json({ ok: false, error: `GitHub API error: ${r.statusText}` });
    }
...
app.post("/api/v1/training/start", async (req, res) => {
  try {
    const r = await trainingCamp.startCamp();
...
app.get("/api/v1/training/status", (req, res) => {
  try {
    res.json({ ok: true, status: trainingCamp.getStatus() });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
app.post("/api/v1/training/round", async (req, res) => {
  try {
    const r = await trainingCamp.runRound();
    res.json({ ok: true, result: r });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
app.get("/api/v1/training/overview", (req, res) => {
  try {
    res.json({ ok: true, data: trainingCamp.getOverviewData() });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
app.get("/api/v1/training/active", (req, res) => {
  try {
    res.json({ ok: true, data: trainingCamp.getActiveTrainingData() });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
app.get("/api/v1/training/deep-dive/:topic", (req, res) => {
  try {
    res.json({
      ok: true,
      data: trainingCamp.getDeepDiveData(req.params.topic),
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
...
app.post("/api/v1/training/new-topic", (req, res) => {
  try {
    ...
    const result = trainingCamp.addTopic({
      key,
      name,
      problems,
      background,
    ...
    return res.json({
      ok: true,
      added: result,
      overview: trainingCamp.getOverviewData(),
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
  ...
  try {
    ...
    const background =
      q.background !== undefined
        ...
        : true;
    const force =
      q.force !== undefined ? String(q.force).toLowerCase() === "true" : false;
    const result = trainingCamp.addTopic({
      key,
      name,
      problems: undefined,
...
app.get("/api/v1/training/settings", (req, res) => {
  try {
    res.json({ ok: true, settings: trainingCamp.getSettings() });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.post("/api/v1/training/force-masteries", async (req, res) => {
  try {
    const body = req.body || {};
...
app.get("/api/v1/training/force-masteries", async (req, res) => {
  try {
    const q = req.query || {};
    const threshold = Number(
...
app.post("/api/v1/training/start", async (req, res) => {
  try {
    ...
    const result = await trainingCamp.startTraining(userId, focusArea, {
      roundCount,
    });
  ...
  try {
    ...
    res.json({
      ...
      message: result.completed
        ? "Training completed"
        : "Round completed, continue to next",
    });
  } catch (error) {
    ...
    const summary = {
      ...
      avgQuality:
        feedbackStore.feedback.length > 0
          ? feedbackStore.feedback.reduce((a, b) => a + b.quality, 0) /
            feedbackStore.feedback.length
          : 0,
      avgRelevance:
        feedbackStore.feedback.length > 0
          ? feedbackStore.feedback.reduce((a, b) => a + b.relevance, 0) /
            feedbackStore.feedback.length
          : 0,
    };
...
app.post("/api/v1/optimization/plan", async (req, res) => {
  try {
    const { task, context } = req.body;

    // 1. Select Provider (Simple logic for now, can be enhanced)
    ...
    const limit = 100000; // Mock limit

    let prompt = task?.original || "";

    // 3. Summarization Trigger
    if (contextLength > limit) {
