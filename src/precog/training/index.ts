...
export class TrainingService extends EventEmitter {
  ...
  constructor(private workspaceRoot: string) {
    super();
    this.sourcesStateFile = path.join(this.workspaceRoot, 'data', 'sources-github-state.json');
      ...
      const newData = {
        ...
        improvements: {
          ...
          firstTrySuccess: {
            ...
            achieved:
              firstTrySuccessRate >= (currentData.improvements?.firstTrySuccess?.target || 0.9),
          },
          ...
          repeatProblems: {
            ...
            achieved:
              0.15 - total * 0.001 <= (currentData.improvements?.repeatProblems?.target || 0.05),
          },
        },
  ...
  async getNextDomain() {
    ...
    const avg = domains.length
      ? domains.reduce((s: number, d: Domain) => s + (d.mastery || 0), 0) / domains.length
      : 0;
    const gaps = domains
  ...
  async startTrainingSession(userId: string, focusArea: string, roundCount?: number) {
  ...
  async completeTrainingRound(roundId: string, response: string, score?: number) {
    return await this.trainingCamp.completeRound(roundId, response, score ?? 0);
  }
  ...
  async startChallenge(userId: string, skill: string, difficulty: string = 'medium') {
  ...
  getProviderFeedback(provider: string) {
    const providerFeedback = this.feedbackStore.feedback.filter((f) => f.provider === provider);
    ...
    return {
      ...
      helpfulRate:
        providerFeedback.filter((f) => f.helpful).length / Math.max(1, providerFeedback.length),
  ...
  recordMetrics(data: MetricsData) {
    ...
    providerData.averageLatency =
      (providerData.averageLatency * (providerData.totalRequests - 1) + latency) /
      providerData.totalRequests;
    providerData.averageQuality =
      (providerData.averageQuality * (providerData.totalRequests - 1) + quality) /
      providerData.totalRequests;
  ...
  getUserProfile(userId: string) {
    const userInteractions = this.feedbackStore.interactions.filter((i) => i.userId === userId);

    return {
      userId,
      totalInteractions: userInteractions.length,
      engagementRate:
        userInteractions.filter((i) => i.engaged).length / Math.max(1, userInteractions.length),
  ...
  getOptimizationPlan(task: OptimizationTask, context: OptimizationContext) {
    ...
    if (taskType === 'creative') provider = 'gemini';
    if (taskType === 'reasoning') provider = 'anthropic';
    if (taskType === 'speed') provider = 'deepseek';
