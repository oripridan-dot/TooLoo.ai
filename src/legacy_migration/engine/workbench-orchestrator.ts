...
export class WorkbenchOrchestrator {
  constructor(options = {}) {
    ...
    this.services = {
      segmentation: { port: 3007, path: '/api/v1/segmentation', status: 'unknown' },
      ...
      capabilities: { port: 3009, path: '/api/v1/capabilities', status: 'unknown' }
    };
    ...
    this.intentPatterns = {
      ...
      prediction: {
        keywords: ['predict', 'next', 'what', 'future', 'should', 'recommend', 'suggest'],
        services: ['meta', 'coach', 'training'],
        description: 'Predicting outcomes and recommending actions'
      },
      learning: {
        keywords: ['learn', 'teach', 'skill', 'master', 'understand', 'training', 'course'],
  ...
  async executeWorkRequest(workId, goal, context = {}, options = {}) {
    ...
    try {
      ...
      this.log(`⚡ Stage 4: Executing ${pipeline.services.length} services in parallel...`);
      const serviceResults = await this.executeServices(pipeline, goal, context);
  ...
  async analyzeIntent(goal, context = {}) {
    ...
    try {
      const result = await this.llmProvider.generate({
        prompt,
        system: "You are the Intent Analyzer for the TooLoo.ai Workbench. Output valid JSON only.",
        taskType: "analysis",
        criticality: "normal"
      });
      ...
      this.log(`  Intent: ${analysis.type} (confidence: ${analysis.confidence})`);
      return { ...analysis, goal, context };
      
    } catch (error) {
      this.log(`  ⚠️ LLM Analysis failed, falling back to regex: ${error.message}`);
  ...
  async analyzeIntentFallback(goal, context = {}) {
    ...
    this.log(`  Intent (Fallback): ${intentType} (confidence: ${topIntent[1]}/100)`);
  ...
  buildPipeline(intent, options = {}) {
    ...
    this.log(`  Execution Order: ${pipeline.executionOrder.map(phase => `[${phase.join(',')}]`).join(' → ')}`);
  ...
  buildExecutionOrder(services, strategy) {
    ...
    const phase1 = services.filter(s => strategy.parallel.includes(s) && !processed.has(s));
    if (phase1.length > 0) {
      order.push(phase1);
    ...
    const phase2 = services.filter(s => strategy.sequential.includes(s) && !processed.has(s));
    if (phase2.length > 0) {
      order.push(phase2);
    ...
    const phase3 = services.filter(s => strategy.final.includes(s) && !processed.has(s));
    if (phase3.length > 0) {
      order.push(phase3);
  ...
  async checkServicesHealth(services) {
    const checks = services.map(async (service) => {
      try {
        const response = await fetch(`${this.baseUrl}${this.services[service].path}/health`, {
          timeout: 5000
        });
  ...
  async executeServices(pipeline, goal, context) {
    ...
    for (let phaseIdx = 0; phaseIdx < pipeline.executionOrder.length; phaseIdx++) {
      ...
      const phasePromises = phase.map(async (service) => {
        try {
          const result = await this.executeService(service, goal, context, results);
      ...
      const successful = phaseResults.filter(r => r.status === 'success').length;
      this.log(`  Phase complete: ${successful}/${phase.length} services successful`);
    }
  ...
  async executeService(service, goal, context, previousResults) {
    ...
    const payload = this.prepareServicePayload(service, goal, context, previousResults);
  ...
  async synthesizeResults(serviceResults, intent) {
    ...
    try {
      ...
      const result = await this.llmProvider.generate({
        prompt,
        system: "You are the Synthesis Engine for TooLoo.ai. Consolidate technical results into clear, actionable insights.",
        taskType: "analysis",
        criticality: "normal"
      });
      ...
      synthesis.findings.push(...(aiSynthesis.keyInsights || []).map(i => ({ source: 'ai-synthesis', insights: [i] })));
      synthesis.recommendations = [...new Set([...synthesis.recommendations, ...(aiSynthesis.recommendations || [])])];
      synthesis.nextSteps = [...new Set([...synthesis.nextSteps, ...(aiSynthesis.nextSteps || [])])];
      synthesis.actionable = this.generateActionableSummary(synthesis);

    } catch (error) {
      this.log(`  ⚠️ LLM Synthesis failed, using basic aggregation: ${error.message}`);
      synthesis.actionable = this.generateActionableSummary(synthesis);
    }
  ...
  generateActionableSummary(synthesis) {
    ...
    if (synthesis.nextSteps.length > 0) {
      summaryParts.push(`Action items: ${synthesis.nextSteps.slice(0, 3).join('; ')}`);
    }
  ...
  async postProcess(synthesized, options = {}) {
    ...
    if (options.createPR && output.githubInfo) {
      try {
        const prResult = await this.createPullRequest(output.githubInfo, synthesized);
  ...
  async commitToGithub(synthesized, options = {}) {
    ...
    const message = options.message || `Workbench: ${synthesized.intent} analysis`;

    try {
      const response = await fetch(`${this.baseUrl}/api/v1/github/update-file`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          path: filename,
