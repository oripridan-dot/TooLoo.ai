...
const hyperCamp = new HyperSpeedTrainingCamp({ workspaceRoot: process.cwd(), trainingCamp });
...
app.post('/api/v1/optimization/plan', async (req, res) => {
  try {
    ...
    if (contextLength > limit) {
      ...
      console.log(`Context length ${contextLength} exceeds limit ${limit}. Triggering summarization.`);
