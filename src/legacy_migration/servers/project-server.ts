...
class ProjectServer {
  ...
  constructor() {
    this.svc = new ServiceFoundation('project-server', process.env.PROJECT_SERVER_PORT ? parseInt(process.env.PROJECT_SERVER_PORT) : 3011);
  ...
  setupRoutes() {
    ...
    this.app.post('/api/v1/projects', async (req, res) => {
      try {
        ...
        await fs.writeFile(path.join(memoryDir, 'long-term.md'), `# Long Term Memory for ${name}\n\n${description || ''}`);
        await fs.writeFile(path.join(memoryDir, 'short-term.md'), `# Short Term Memory for ${name}\n\n- Project initialized`);
        ...
        res.json({ ok: true, project: config, message: 'Project created successfully' });
      } catch (error: any) {
    ...
    this.app.get('/api/v1/projects/:id', async (req, res) => {
      try {
        ...
        const configPath = path.join(projectPath, 'tooloo.json');
        const config = await fs.pathExists(configPath) ? await fs.readJson(configPath) : { id, name: id };
        
        // Read memory
        const memoryDir = path.join(projectPath, '.memory');
        const longTerm = await fs.pathExists(path.join(memoryDir, 'long-term.md')) ? await fs.readFile(path.join(memoryDir, 'long-term.md'), 'utf8') : '';
        const shortTerm = await fs.pathExists(path.join(memoryDir, 'short-term.md')) ? await fs.readFile(path.join(memoryDir, 'short-term.md'), 'utf8') : '';

        res.json({ ok: true, project: config, memory: { longTerm, shortTerm } });
      } catch (error: any) {
        res.status(500).json({ ok: false, error: error.message });
      }
    });
    ...
    this.app.get('/api/v1/projects/:id/tasks', async (req, res) => {
      try {
        ...
        const tasks = await fs.pathExists(tasksPath) ? await fs.readJson(tasksPath) : [];
    ...
    this.app.get('/api/v1/projects/:id/files', async (req, res) => {
      try {
        ...
        if (!targetPath.startsWith(projectPath)) return res.status(403).json({ error: 'Access denied' });
    ...
    this.app.get('/api/v1/projects/:id/files/content', async (req, res) => {
      try {
        ...
        if (!targetPath.startsWith(projectPath)) return res.status(403).json({ error: 'Access denied' });
        if (!await fs.pathExists(targetPath)) return res.status(404).json({ error: 'File not found' });
    ...
    this.app.get('/api/v1/projects/:id/git/log', async (req, res) => {
      try {
        ...
        const { stdout } = await execAsync('git log --pretty=format:"%h - %an, %ar : %s" -n 10', { cwd: projectPath });
