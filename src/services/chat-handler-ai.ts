...
export async function handleChatWithAI(message, options = {}) {
  try {
    // 1. Hydration: Get Context & Persona
    const sessionId = options.sessionId || 'default-session';
    const [contextData, persona] = await Promise.all([
      apiCall(
        3006,
        'GET',
        `/api/v1/product/session/${sessionId}/context?message=${encodeURIComponent(message)}`
      ).catch(() => ({ context: { history: [], contextInjection: '' } })),
      apiCall(META_PORT, 'GET', '/api/v1/meta/persona').catch(() => ({ persona: {} })),
    ]);

    const history = contextData.context?.history || [];
    const contextInjection = contextData.context?.contextInjection || '';
    const projectId = contextData.context?.projectId;

    // Use the intelligent context injection as the primary context
    let contextString = contextInjection;

    // 1.5 Project Context Injection
    if (projectId) {
      try {
        const projectData = await apiCall(PROJECT_PORT, 'GET', `/api/v1/projects/${projectId}`);
        if (projectData.ok && projectData.memory) {
          contextString += `\n\n## PROJECT CONTEXT: ${projectData.project.name}\n`;
          if (projectData.memory.longTerm) {
            contextString += `\n### Long Term Memory\n${projectData.memory.longTerm}\n`;
          }
          if (projectData.memory.shortTerm) {
            contextString += `\n### Short Term Memory\n${projectData.memory.shortTerm}\n`;
          }
        }
      } catch (e) {
        console.warn(`Failed to fetch project context for ${projectId}:`, e.message);
      }
    }

    const userPersona = persona.persona || {};

    // 2. Normalization: Plan Execution
    const taskType = detectTaskType(message);
    const plan = await apiCall(TRAINING_PORT, 'POST', '/api/v1/optimization/plan', {
      task: { original: message, type: taskType },
      context: history,
    });

    // 3. Execution: Route to Provider
    ...
    // Check for direct GitHub commands first (Fast Path)
    const directResponse = await handleGitHubCommand(message);
    if (directResponse) {
      return directResponse;
    }

    // Execute with best-fit provider(s) based on task type
    let response = await executeWithBestProvider(
      message,
      contextString,
      taskType,
      plan.plan?.provider,
    ...
    if (response) {
      ...
      // Async save
      apiCall(3006, 'POST', '/api/v1/product/session', {
        sessionId,
        messages: newHistory,
        metadata: { lastProvider: plan.plan?.provider },
...
function detectTaskType(message) {
  ...
  if (
    /execute|can you.*execute|run.*code|implement.*changes|apply.*directly|execute.*suggestion/.test(
      lower
    )
  ) {
    return 'execution'; // Anthropic (newer) or OpenAI for action-oriented responses
  }
  if (/github|repo|branch|pull request|pr|issue|commit/.test(lower)) {
    return 'github'; // GitHub operations
  }
 ...
 */
async function executeWithBestProvider(
  message: string,
  context: string,
  taskType: string,
  plannedProvider: string | null = null,
  overrideSystemPrompt: string | null = null,
  userPersona: any = {}
) {
  const providerSequence = buildProviderSequence(taskType);

  // Move planned provider to front if it exists in sequence
  if (plannedProvider && providerSequence.includes(plannedProvider)) {
    const idx = providerSequence.indexOf(plannedProvider);
    providerSequence.splice(idx, 1);
    providerSequence.unshift(plannedProvider);
  }
  ...
  for (const provider of providerSequence) {
    try {
      ...
      switch (provider) {
        case 'ollama':
          response = await callProviderWithContext(
            provider,
            message,
            context,
        ...
        case 'anthropic':
          ...
          if (
            response &&
            (taskType === 'optimization' || taskType === 'reasoning' || taskType === 'execution')
          ) {
...
function buildProviderSequence(taskType) {
  const sequences = {
    ...
    github: ['gemini', 'openai', 'anthropic', 'deepseek', 'ollama'],
...
async function callProviderWithContext(
  ...
  // Inject Persona
  let personaContext = '';
  if (userPersona.systemInstructions && Array.isArray(userPersona.systemInstructions)) {
    personaContext = `\n\nUser Preferences:\n${userPersona.systemInstructions.join('\n')}`;
  }
  ...
  try {
    ...
    const request = {
      prompt: message,
      system: systemPrompt + fullContext + resonanceContext + personaContext,
      taskType: detectTaskType(message),
      maxTokens: 1000,
    ...
    switch (provider) {
      case 'anthropic':
        const claudeResult = await providerInstance.callClaude(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        response = claudeResult?.content || claudeResult;
        break;
      case 'openai':
        const openaiResult = await providerInstance.callOpenAI(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        ...
        break;
      case 'gemini':
        const geminiResult = await providerInstance.callGemini(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        ...
        break;
      case 'ollama':
        const ollamaResult = await providerInstance.callOllama(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        ...
        break;
      case 'deepseek':
        const deepseekResult = await providerInstance.callDeepSeek(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        ...
        break;
      case 'localai':
        const localaiResult = await providerInstance.callLocalAI(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
      ...
      case 'openinterpreter':
        const oiResult = await providerInstance.callOpenInterpreter(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
        ...
        break;
      case 'huggingface':
        const hfResult = await providerInstance.callHuggingFace(
          message,
          systemPrompt + fullContext + resonanceContext + personaContext
        );
...
async function handleChatWithPatterns(message) {
  ...
  // GitHub Commands
  const githubResponse = await handleGitHubCommand(message);
...
async function handleGitHubCommand(message) {
  // Create Branch: "create branch <name> [from <base>]"
  const branchMatch = message.match(/create branch ([\w-]+)(?: from ([\w-]+))?/i);
  if (branchMatch) {
    const [_, name, from] = branchMatch;
  ...
  // Create PR: "create pr <title> from <branch>"
  const prMatch = message.match(/create (?:pr|pull request) (.+?) from ([\w-]+)/i);
  if (prMatch) {
    const [_, title, head] = prMatch;
    const result = await githubProvider.createPullRequest({
      title,
      body: 'Created via TooLoo Chat',
      head,
      base: 'main',
    });
  ...
  const issueMatch = message.match(/create issue (.+)/i);
  if (issueMatch) {
  ...
  // Autonomous Workflow (No-Code Mode)
  // Matches: "start working on <task>", "implement <feature>", "fix <bug>"
  const workflowMatch =
    message.match(/^(?:start|begin) (?:working on|building|developing) (.+)/i) ||
    message.match(/^(?:implement|add) (?:feature|capability) (.+)/i) ||
    message.match(/^(?:fix) (?:bug|issue|problem) (.+)/i);

  if (workflowMatch) {
    const taskDescription = workflowMatch[1];
    const timestamp = Date.now().toString().slice(-4);
    const slug = taskDescription
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '');
    const branchName = `feature/${slug}-${timestamp}`;

    // 1. Create Branch
    const branchResult = await githubProvider.createBranch(branchName, 'main');

    if (!branchResult.success) {
      return `❌ Could not start workspace: ${branchResult.error}`;
    }

    // 1.5 Create an initial commit to allow PR creation
    await githubProvider.createOrUpdateFile(
      `workspace-init-${timestamp}.md`,
      `# Workspace: ${taskDescription}\n\nInitialized: ${new Date().toISOString()}\n\nThis file marks the start of the workspace.`,
      `chore: Initialize workspace for ${taskDescription}`,
      branchName
    );

    // 2. Create PR (Draft)
    const prResult = await githubProvider.createPullRequest({
      title: `Feature: ${taskDescription}`,
      body: `## Automated Workspace\n\n**Task:** ${taskDescription}\n**Status:** In Progress\n\nManaged by TooLoo.ai Autonomous Workflow.`,
      head: branchName,
      base: 'main',
      draft: true,
      labels: ['automated-workflow', 'in-progress'],
    });

    if (prResult.success) {
      return (
        `🚀 **Workspace Initialized**\n\nI've set up a dedicated environment for **"${taskDescription}"**.\n\n` +
        `• **Workspace ID:** \`${branchName}\`\n` +
        `• **Tracking:** [View Progress](${prResult.html_url})\n` +
        `• **Status:** Ready for development\n\n` +
        `You can now give me instructions like "Create the file..." or "Update the code..." and I'll handle the technical details.`
      );
    } else {
      return `⚠️ Workspace created (Branch: ${branchName}) but tracking setup failed: ${prResult.error}`;
    }
  }
...
async function getResonanceContext(message) {
  try {
    ...
    const result = await apiCall(
      SEGMENTATION_PORT,
      'POST',
      '/api/v1/segmentation/resonance',
      {
        context: message,
        traits: traits,
      },
      1000
    ); // Short timeout to not block chat
...
async function getDomains() {
  try {
    ...
    response += domains
      .map((d) => {
        const bar =
          '█'.repeat(Math.floor(d.mastery / 10)) + '░'.repeat(10 - Math.floor(d.mastery / 10));
...
async function coach(intensity = 'normal') {
  try {
    ...
    const mode =
      intensity === 'slow' ? 'supportive' : intensity === 'fast' ? 'accelerated' : 'normal';
...
async function focusOn(domain, message) {
  try {
    ...
    const overview = await apiCall(TRAINING_PORT, 'GET', '/api/v1/training/overview');
    const current = overview.data?.domains?.find((d) =>
    ...
    return (
      `✅ Focused Optimization on ${fullDomain}\n\n` +
...
async function optimize() {
  try {
    ...
    return (
      `✅ Meta-Acquisition Optimization Complete\n\n` +
      `Retention Boost: +15%\n` +
