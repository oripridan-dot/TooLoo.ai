...
const INSTRUCTIONS_FILE = path.join(process.cwd(), '.github/copilot-instructions.md');
...
async function generateContext() {
  ...
  const srcFiles = await glob('src/**/*.{ts,tsx,js,jsx}', { ignore: ['**/*.test.ts', '**/*.spec.ts', '**/node_modules/**'] });
  ...
  for (const [key, files] of Object.entries(structureMap)) {
    ...
    if (files.length > 20) {
        structureMd += files.slice(0, 10).map(f => `  - ${f}`).join('\n') + `\n  - ... and ${files.length - 10} more\n`;
    } else {
