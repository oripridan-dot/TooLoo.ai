...
export class Orchestrator {
  ...
  async process(
    ...
    eventBus.publish('engine', 'orchestration:complete', {
      sessionId: this.contextBuilder.getSessionId(),
  ...
  private detectIntent(message: string): { type: string; confidence: number } {
    ...
    const patterns: Array<{ type: string; keywords: string[]; weight: number }> = [
      ...
      { type: 'debug', keywords: ['debug', 'fix', 'error', 'bug', 'issue', 'problem'], weight: 0.85 },
      ...
      { type: 'deploy', keywords: ['deploy', 'release', 'ship', 'publish', 'launch'], weight: 0.9 },
