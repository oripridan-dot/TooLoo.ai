...
export class ModelChooser {
  constructor(options = {}) {
    this.config = {
      ...
      budgetCapUsd: options.budgetCapUsd || 0.50,
      ...options
    };
  ...
  analyzeComplexity(intent) {
    ...
    return {
      ...
      isCreative,
      isValidation,
      taskType: isCode ? 'code' : isResearch ? 'research' : isCreative ? 'creative' : 'general'
    };
  }
  ...
  selectProvidersForLane(lane, taskType = 'general') {
    ...
    if (lane === 'fast') {
      ...
      return available.slice(0, Math.min(3, available.length)).map(([name]) => name);
    }
  ...
  buildExecutionPlan(intent, budgetUsd = null) {
    ...
    if (budget < 0.05 || complexity.complexity === 'simple') {
      lanes = ['fast'];
    } else if (complexity.complexity === 'complex' && budget > 0.20) {
    ...
    for (const lane of lanes) {
      ...
      const candidates = [];
      for (let i = 0; i < Math.min(this.config.tournamentSize, providers.length); i++) {
    ...
    if (plan.totalEstimatedCostUsd > budget) {
      ...
      plan.totalEstimatedCostUsd = (plan.lanes.fast || []).reduce((sum, c) => sum + c.estimatedCostUsd, 0);
    }
