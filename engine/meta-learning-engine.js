...
import PerMetricStrategies from './per-metric-strategies.js';
...
export default class MetaLearningEngine {
  ...
  async phase1_LearningAnalysis() {
    ...
    if (realMetrics) {
      ...
      findings.push({ type: 'simulated_analysis', reason: 'training-server unavailable' });
    }

    this.phases[1].findings = findings;
    this.log('Phase 1 analysis complete', { metricsSource: realMetrics ? 'real' : 'simulated', metrics: this.metrics.current });
  }
  ...
  async reset() {
    ...
    this.metrics.improvements.push({ phase: 'system', metric: 'reset', delta: 0, at: new Date().toISOString() });
    
    // Clear activity log but log the reset
    this.activityLog = [{ time: new Date().toISOString(), message: 'Meta-Learning Engine reset to initial state' }];
  ...
  predictPlateauTiming(confidenceThreshold = 0.65) {
    ...
    return {
      ...
      forecast: forecast.forecast,
      recommendedAction: confidence >= confidenceThreshold ? 'trigger_preemptive_strategy' : 'monitor'
    };
  }
  ...
  async triggerAdaptiveStrategy(mode = 'aggressive') {
    ...
    if (mode === 'aggressive') {
      ...
      const asBoost = 0.20; // +20%
      const krBoost = 0.15; // +15%
      const teBoost = 0.15; // +15%

      this.metrics.current.learningVelocity = Math.min(1.0, (this.metrics.current.learningVelocity || 0) + lvBoost);
      this.metrics.current.adaptationSpeed = Math.min(1.0, (this.metrics.current.adaptationSpeed || 0) + asBoost);
      this.metrics.current.knowledgeRetention = Math.min(1.0, (this.metrics.current.knowledgeRetention || 0) + krBoost);
      this.metrics.current.transferEfficiency = Math.min(1.0, (this.metrics.current.transferEfficiency || 0) + teBoost);

      result.actions.push(
        { action: 'boost_learning_velocity', delta: lvBoost, newValue: this.metrics.current.learningVelocity },
        { action: 'boost_adaptation_speed', delta: asBoost, newValue: this.metrics.current.adaptationSpeed },
        { action: 'boost_retention', delta: krBoost, newValue: this.metrics.current.knowledgeRetention },
        { action: 'boost_transfer', delta: teBoost, newValue: this.metrics.current.transferEfficiency }
      );
   ...
   */
  async applyPerMetricStrategies(confidenceThreshold = 0.70) {
  ...
  async fetchRealMetricsFromTraining() {
    try {
      ...
      const avgMastery = domains.length > 0 
        ? domains.reduce((sum, d) => sum + (d.mastery || 0), 0) / domains.length 
        : 0.35;
      ...
      this.log('Failed to fetch real metrics from training-server', { error: error.message });
      return null;
    }
  }
  ...
  async phase1_LearningAnalysisEnhanced() {
    ...
    this.phases[1].findings = findings;
    this.log('Phase 1 analysis complete (enhanced)', { metricsSource: realMetrics ? 'real' : 'simulated' });
  }
  ...
  async checkPlateauPredictiveAndReactive(confidenceThreshold = 0.70) {
  ...
  async checkPlateauAndAutoAdapt() {
    ...
    if (check.plateauDetected) {
      this.log('Auto-adapting due to plateau', {
        ...
        usePerMetric: !!this.perMetricStrategies && check.confidence > 0.70
      });

      // Phase 2B: Use per-metric strategies if available and confident
      if (this.perMetricStrategies && check.confidence > 0.70) {
