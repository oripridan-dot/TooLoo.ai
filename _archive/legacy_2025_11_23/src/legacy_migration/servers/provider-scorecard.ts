...
export class ProviderScorecard {
  ...
  async collectProviderMetrics() {
    try {
      ...
      if (!response.ok)
        throw new Error(`Budget service returned ${response.status}`);
  ...
  calculateScores(providers) {
    return providers.map((p) => {
      ...
      const overallScore =
        latencyScore * 0.4 + successScore * 0.4 + costScore * 0.2;
    ...
    return this.history.slice(-limit).map((h) => ({
      ...
      averageScore:
        h.leaderboard.length > 0
          ? Math.round(
              h.leaderboard.reduce(
                (sum, p) => sum + (p.scores?.overall || 0),
                0
    ...
    if (trends.length > 0) {
      ...
      if (prevTrend && lastTrend.averageScore < prevTrend.averageScore) {
        insights.push({
          ...
          message:
            "Overall provider performance has declined. Consider rotating providers or investigating issues.",
        });
      }
    }
