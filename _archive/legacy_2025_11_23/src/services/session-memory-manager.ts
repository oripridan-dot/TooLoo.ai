...
import ConversationMemoryEngine from "../cortex/engine/conversation-memory-engine.js";
import { getContextInjectionEngine } from "../nexus/engine/context-injection-engine.js";
...
export class SessionMemoryManager {
      ...
      for (const file of files) {
        if (file.endsWith(".json") && file !== "sessions.json") {
          try {
            ...
            console.warn(
              `[SessionMemory] Failed to load session file ${file}:`,
              e.message
            );
          }
        }
  ...
  getSessionContext(sessionId) {
    ...
    const topics = [
      ...new Set(recent.map((m) => this.extractTopics(m.content)).flat()),
    ];
    ...
    return {
      ...
      averageResponseTime:
        assistantMessages.length > 0
          ...
          : 0,
      providers: [
        ...new Set(assistantMessages.map((m) => m.provider).filter(Boolean)),
      ],
  ...
  async updateSessionMetadata(sessionId, metadata = {}) {
    ...
    if (
      metadata.provider &&
      !session.context.providers.includes(metadata.provider)
    ) {
