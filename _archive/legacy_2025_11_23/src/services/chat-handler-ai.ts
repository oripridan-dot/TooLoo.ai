...
import { IntentPacket } from "../nexus/engine/intent-bus.js";
import { ModelChooser } from "../nexus/engine/model-chooser.js";
import LLMProvider from "../precog/providers/llm-provider.js";
import githubProvider from "../nexus/engine/github-provider.js";
...
export async function handleChatWithAI(message, options = {}) {
  try {
    ...
    const [contextData, persona] = await Promise.all([
      ...
      apiCall(META_PORT, "GET", "/api/v1/meta/persona").catch(() => ({
        persona: {},
      })),
    ]);
    ...
    try {
      ...
      console.warn(
        "Could not initialize ModelChooser/LLMProvider/Instructions:",
        initErr.message
      );
    ...
    try {
      ...
      switch (provider) {
        ...
        case "gemini":
          ...
          if (
            response &&
            (taskType === "creative" || taskType === "research")
          ) {
...
async function callProviderWithContext(
  ...
  let personaContext = "";
  if (
    userPersona.systemInstructions &&
    Array.isArray(userPersona.systemInstructions)
  ) {
...
async function getLearnerContext() {
  try {
    ...
    const avg = Math.floor(
      domains.reduce((a, b) => a + b.mastery, 0) / domains.length
    );
...
async function handleGitHubCommand(message) {
  ...
  const prMatch = message.match(
    /create (?:pr|pull request) (.+?) from ([\w-]+)/i
  );
  if (prMatch) {
  ...
  const workflowMatch =
    message.match(
      /^(?:start|begin) (?:working on|building|developing) (.+)/i
    ) ||
...
async function getDomains() {
  try {
    ...
    response += domains
      .map((d) => {
        const bar =
          "█".repeat(Math.floor(d.mastery / 10)) +
          "░".repeat(10 - Math.floor(d.mastery / 10));
...
async function coach(intensity = "normal") {
  try {
    ...
    const boost = await apiCall(
      COACH_PORT,
      "POST",
      "/api/v1/auto-coach/boost",
      {
        rounds,
        mode,
      }
...
async function optimize() {
  try {
    const meta = await apiCall(
      META_PORT,
      "POST",
      "/api/v4/meta-learning/boost-retention",
      {
        retentionDelta: 0.15,
        transferDelta: 0.1,
      }
