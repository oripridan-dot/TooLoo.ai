...
export default class ProviderAggregation {
  ...
  async callAllProviders(basePrompt, context = {}) {
    if (!this.instructions) {
      throw new Error(
        "Aggregation system not initialized. Call initialize() first."
      );
    }
    ...
    const calls = providers.map((providerInstr) =>
      this.callSingleProvider(providerInstr, basePrompt, context)
    );
    ...
    try {
      ...
      console.error(
        "[ProviderAggregation] Error calling providers:",
        err.message
      );
      throw err;
    }
  ...
  async callSingleProvider(providerInstr, basePrompt, context) {
    ...
    try {
      ...
      if (!this.llmProvider.providers[providerKey]) {
        throw new Error(
          `Provider ${providerInstr.name} not available (missing API keys)`
        );
      }
  ...
  getProviderAnalysis() {
    ...
    return {
      summary: {
        ...
        failed: failed.length,
        successRate:
          ((successful.length / this.results.length) * 100).toFixed(1) + "%",
      },
      timing: {
