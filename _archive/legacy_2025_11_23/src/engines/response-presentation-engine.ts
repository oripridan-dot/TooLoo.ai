...
Return ONLY valid JSON, no markdown.`;
  ...
  async createAdvisory(query, consensus, conflicts, actions, userContext = {}) {
    const contextText = userContext.role
...
Return ONLY valid JSON, no markdown.`;
    ...
    try {
      ...
      return {
        ...
        nextMoves: [
          ...
          "Take priority action items in order",
        ],
  ...
  formatTooLooResponse(data) {
    ...
    const actionsList = actions
      ...
      .map(
        (a) => `${a.priority}. **${a.action}** _(${a.effort})_ — ${a.rationale}`
      )
      .join("\n");
    ...
    const riskBadge = advisory.riskFlag
      ? `🚩 **RISK**: ${advisory.riskFlag}\n\n`
      : "";
