...
export class ServiceFoundation {
  constructor(serviceName, port = null, options = {}) {
    ...
    // Use shared environment hub for component registration
    this.environmentHub = environmentHub;
  ...
  setupMiddleware(options = {}) {
    ...
    this.app.use((req, res, next) => {
      ...
      const env = this.env;
      const endpoints = this.metrics.endpoints;
      ...
      res.send = function (data) {
        ...
        if (!endpoints[endpoint]) {
          endpoints[endpoint] = { count: 0, totalLatency: 0, errors: 0 };
        }
        endpoints[endpoint].count++;
        endpoints[endpoint].totalLatency += duration;

        if (res.statusCode >= 400) {
          metrics.errors++;
          endpoints[endpoint].errors++;
          const code = res.statusCode;
          metrics.errors_by_code[code] =
            (metrics.errors_by_code[code] || 0) + 1;
        } else {
        ...
        if (env === "development") {
          ...
          console.log(
            `[${serviceName}] ${status} ${req.method} ${req.path} ${res.statusCode} +${duration}ms`
          );
        }
  ...
  errorHandler(err, req, res, next) {
    ...
    console.error(
      `[${this.serviceName}] ${req.method} ${req.path} - ${err.message}`
    );
  ...
  async checkDependencies() {
    for (const dep of this.dependencies) {
      try {
        const response = await fetch(`http://127.0.0.1:${dep.port}/health`, {
          timeout: 2000,
        });
    ...
    const allCriticalHealthy = criticalDeps.every((d) =>
      this.dependencyHealth.get(d.name)
    );
  ...
  async start() {
    return new Promise((resolve) => {
      ...
      this.server = this.app.listen(this.port, "127.0.0.1", () => {
        console.log(
          `✨ [${this.serviceName}] ready on http://127.0.0.1:${this.port}`
        );
  ...
  async stop() {
    return new Promise((resolve) => {
      if (this.server) {
        ...
        setTimeout(() => {
          console.warn(
            `⚠️  [${this.serviceName}] forcing shutdown after timeout`
          );
          process.exit(1);
        }, 10000);
      } else {
