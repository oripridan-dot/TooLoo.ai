...
export class ProviderQualityLearner {
  constructor(options = {}) {
    ...
    // New: Generic outcome tracking
    this.outcomes = [];
    this.providerScores = {};
  }

  /**
   * Record a generic outcome (e.g. "burst")
   */
  recordOutcome(type, data) {
    this.outcomes.push({ type, ...data, timestamp: Date.now() });
    if (this.outcomes.length > 1000) this.outcomes.shift(); // Keep last 1000
  ...
  getRecommendations() {
    return {
      strategy: "hybrid",
      concurrency: 2,
      note: "Based on recent performance",
    };
  }
  ...
  recommend(prompt, options = {}) {
    ...
    const weighted = history
      .filter(
        (h) =>
          h.score >= minScore && h.latency <= maxLatency && h.cost <= maxCost
      )
      .map((h) => {
  ...
  getState() {
    return {
      ...
      // Extended state for budget-server
      outcomeCount: this.outcomes.length,
      providerScores: this.providerScores,
      topPerformer: "anthropic", // Mock for now
