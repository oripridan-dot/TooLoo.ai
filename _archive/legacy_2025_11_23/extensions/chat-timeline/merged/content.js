...
class ChatTimelineNavigator {
  ...
  async waitForPlatformMount(timeoutMs = 8000) {
    ...
    while (performance.now() - start < timeoutMs) {
      ...
      if (selectors.some(sel => document.querySelector(sel)) || this.findExistingSidebar()) {
        console.log('[Timeline] ChatGPT platform mounted');
        return;
      }
      await new Promise(r => setTimeout(r, 200));
    }
    console.warn('[Timeline] ChatGPT platform mount timeout');
  }

  async setup() {
    // Wait for platform to be ready (especially ChatGPT lazy loading)
  ...
  loadPersisted() {
    try {
      const stored = localStorage.getItem('timeline_sessions');
      if (stored) {
        this.persistedSessions = JSON.parse(stored);
        console.log('[Timeline] Loaded', this.persistedSessions.length, 'saved sessions');
      }
    } catch (e) {
      console.warn('[Timeline] Failed to load persisted sessions:', e);
    }
  }
  ...
  savePersisted() {
    try {
      ...
      if (this.persistedSessions.length > maxSessions) {
        this.persistedSessions = this.persistedSessions.slice(-maxSessions);
      }
      localStorage.setItem('timeline_sessions', JSON.stringify(this.persistedSessions));
      console.log('[Timeline] Saved', this.persistedSessions.length, 'sessions');
    } catch (e) {
      console.warn('[Timeline] Failed to save sessions:', e);
    }
  }
      ...
      <div class="ai-timeline-content active">
        <div class="ai-timeline-header">
          ...
          <div class="ai-timeline-controls">
            <button id="ai-timeline-history" class="ai-timeline-btn-small" title="History">
              🕓
            </button>
  ...
  attachEventListeners() {
    document.getElementById('ai-timeline-history')?.addEventListener('click', () => {
  ...
  showHistoryModal() {
    if (this.persistedSessions.length === 0) {
      alert('No saved timeline sessions yet. Sessions are automatically saved as you chat.');
      return;
    }

    // Create simple history modal
    const modal = document.createElement('div');
    modal.className = 'ai-timeline-modal';
    modal.innerHTML = `
      <div class="ai-timeline-modal-content">
        <div class="ai-timeline-modal-header">
          <h3>📚 Timeline History (${this.persistedSessions.length} sessions)</h3>
          <button class="ai-timeline-modal-close">✕</button>
        </div>
        <div class="ai-timeline-modal-body">
          ${this.persistedSessions.slice().reverse().map((session, idx) => `
            <div class="history-session-item">
              <div class="session-title">${session.title || 'Untitled Session'}</div>
              <div class="session-meta">${session.segments?.length || 0} segments • ${new Date(session.timestamp).toLocaleDateString()}</div>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    modal.querySelector('.ai-timeline-modal-close')?.addEventListener('click', () => {
      modal.remove();
    });
  ...
  renderSegments(segments) {
    ...
    if (!container) {
      ...
      setTimeout(() => this.renderSegments(segments), 500);
      return;
    }
