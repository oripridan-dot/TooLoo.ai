...
class TemplateEngine {
  ...
  generateFlashcards() {
    ...
    if (container) {
      ...
      container.innerHTML = \`
        <div class="g3-card" style="text-align: center; padding: 2rem; position: relative;">
          <div style="font-size: 0.875rem; color: var(--g3-text-tertiary); margin-bottom: 0.5rem;">FLASHCARD 1/3</div>
          <div class="flashcard-content">
            <div class="front" style="font-size: 1.25rem; font-weight: 600; margin-bottom: 1rem;">What is the primary goal of Phase 2?</div>
            <div class="back" style="display:none; font-size: 1rem; color: var(--g3-text-secondary); margin-bottom: 1rem;">To implement the 4-Panel Workstation and Repository Auto-Organization.</div>
          </div>
          <button class="g3-btn" onclick="this.previousElementSibling.querySelector('.back').style.display = this.previousElementSibling.querySelector('.back').style.display === 'none' ? 'block' : 'none'">Flip Card</button>
        </div>
      \`;
      // Scroll to artifacts
      document.querySelector('.ws-area-artifacts')?.scrollIntoView({ behavior: 'smooth' });
    } else {
      alert('Flashcards generated! (Open Workstation view to see them)');
    }
  }
  ...
  generateConceptMap() {
    ...
    if (container) {
      container.style.display = 'block';
      // In a real app, this would be dynamic based on chat content
      const mermaidDefinition = \`
        graph TD
          A[Project Goal] --> B[Workstation UI]
          A --> C[Repo Automation]
          B --> D[Task Board]
          B --> E[Chat Interface]
          B --> F[Artifacts Panel]
          C --> G[Stale Branch Cleanup]
          C --> H[PR Triage]
      \`;
      ...
      alert('Concept Map generated! (Open Workstation view to see it)');
    }
  }
  ...
  showIdeaBoard() {
    console.log('📌 Opening Idea Board...');
    // Switch to artifacts panel and show board
    const container = document.getElementById('artifacts-container');
    if (container) {
      container.innerHTML = \`
        <div class="g3-card">
          <div class="card-header">Idea Board</div>
          <div class="grid-2">
            <div class="g3-card" style="background:rgba(255,255,255,0.05)">Idea 1</div>
            <div class="g3-card" style="background:rgba(255,255,255,0.05)">Idea 2</div>
            <div class="g3-card" style="background:rgba(255,255,255,0.05)">Idea 3</div>
            <div class="g3-card" style="border:1px dashed var(--g3-border-color)">+ Add Idea</div>
          </div>
        </div>
      \`;
    }
  ...
  showVotingInterface() {
    ...
    alert('Voting Interface requires multi-user session.');
  }
