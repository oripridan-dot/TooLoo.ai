    ...
    const req = http.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          // Unwrap response if it's wrapped in content object
          const body = parsed && parsed.content ? parsed.content : parsed;
          resolve({
...
async function runHealthTests() {
  ...
  await test("GET /health - returns 200 with ok:true", async () => {
    ...
    assert(
      res.body.service === "web-server" || res.body.server === "web",
      "Expected service:web-server"
    );
  });
...
async function runSystemTests() {
  ...
  await test("POST /api/v1/system/priority/background - sets background priority", async () => {
    ...
    if (res.status === 200) {
      assert(res.body.ok === true, "Expected ok:true");
      assert(
        res.body.mode === "background-priority",
        "Expected mode:background-priority"
      );
    }
  });

  await test("GET /api/v1/status - returns system status", async () => {
    const res = await request("GET", "/api/v1/status");
    assert.equal(res.status, 200, `Expected 200, got ${res.status}`);
    assert(res.body.ok === true, "Expected ok:true");
    assert(res.body.uptime !== undefined, "Expected uptime");
  });
}
...
async function runUITests() {
  ...
  await test("GET / - loads dashboard", async () => {
    ...
    assert.equal(res.status, 200, `Expected 200, got ${res.status}`);
    assert(
      res.raw.includes("html") || res.raw.includes("HTML"),
...
async function runChatTests() {
  ...
  await test("GET /api/v1/chat/transcripts - retrieves transcripts", async () => {
    ...
    assert(res.body.ok === true, "Expected ok:true");
    assert(
      Array.isArray(res.body.files) || Array.isArray(res.body.messages),
      "Expected files or messages array"
    );
  });
  ...
  await test("GET /api/v1/chat/burst-stream - streams burst data", async () => {
    const res = await request(
      "GET",
      "/api/v1/chat/burst-stream?prompt=test&limit=10"
    );
    // Stream returns text/event-stream, so body might be null (JSON parse fail) or partial
    assert.equal(res.status, 200, `Expected 200, got ${res.status}`);
  });
}
...
async function runFeedbackTests() {
  ...
  await test("POST /api/v1/referral/create - creates referral", async () => {
    const res = await request("POST", "/api/v1/referral/create", {
      name: "Test User",
      email: "test@example.com",
      userId: "test-user-" + Date.now(), // Add userId
    });
    assert(
      res.status === 200 || res.status === 400 || res.status === 500,
      `Unexpected status ${res.status}`
