...
async function runHealthTests() {
  ...
  await test("GET /health - returns 200", async () => {
    ...
    assert.equal(res.body.service, "training-server");
  });
...
async function runTopicTests() {
  ...
  await test("GET /api/v1/training/new-topic - retrieves created topic", async () => {
    ...
    // Can return 400 if topic already exists
    assert(res.status === 200 || res.status === 400);
    if (res.status === 200) {
...
async function runSettingsTests() {
  ...
  await test("GET /api/v1/training/settings - retrieves settings", async () => {
    ...
    assert.equal(res.status, 200);
    assert(
      res.body.settings || res.body.ok,
      "Expected settings or ok response"
    );
  });
  ...
  await test("GET /api/v1/training/settings/update - updates via query params", async () => {
    const res = await request(
      "GET",
      "/api/v1/training/settings/update?autoFillGaps=true&gapsCount=2"
    );
    assert.equal(res.status, 200);
  });
...
async function runHyperSpeedTests() {
  ...
  await test("POST /api/v1/training/hyper-speed/micro-batch - runs micro batch", async () => {
    const res = await request(
      "POST",
      "/api/v1/training/hyper-speed/micro-batch",
      {
        batchSize: 5,
        topics: ["javascript", "python"],
      }
    );
    // Can return 503/500 if validation fails or safe mode
    assert(
      res.status === 200 ||
        res.status === 400 ||
        res.status === 503 ||
  ...
  await test("POST /api/v1/training/hyper-speed/turbo-round - runs turbo round", async () => {
    const res = await request(
      "POST",
      "/api/v1/training/hyper-speed/turbo-round"
    );
    assert.equal(res.status, 200);
  });
...
async function runProviderTests() {
  ...
  await test("GET /api/v1/providers/parallel-performance - gets performance data", async () => {
    ...
    // Can return 500 if orchestrator not ready
    assert(res.status === 200 || res.status === 500);
    if (res.status === 200) {
      assert(
        res.body.report || res.body.ok,
        "Expected performance or ok response"
      );
    }
  });
}
