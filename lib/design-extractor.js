...
export class DesignExtractor {
  ...
  async _extractColors(html) {
    ...
    // Extract all hex colors (#RRGGBB format)
    ...
    // Extract from CSS property assignments
    const cssColorMatches = html.matchAll(/(?:color|background|border|fill|stroke):\s*([#a-zA-Z0-9]+)/gi);
    ...
    // Extract from data attributes and style values
    const attrMatches = html.matchAll(/(?:data-|style=)[^>]*?(#[0-9a-fA-F]{6})/gi);
  ...
  async _extractSpacing(html) {
    ...
    // Extract all px values from CSS (more aggressive)
    const pxMatches = html.matchAll(/:\s*(\d+)px\b/g);
    for (const match of pxMatches) {
      const px = parseInt(match[1]);
      if (px > 0 && px < 300 && !seen.has(px)) {
        seen.add(px);
        spacing[px] = true;
      }
    }

    // Extract from width/height patterns
    const sizeMatches = html.matchAll(/(?:width|height|min-width|max-width):\s*(\d+)px/gi);
