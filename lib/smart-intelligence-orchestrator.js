  ...
  runFullPipeline(responseText, crossValidationResult, analysisResult, technicalValidationResult, context = {}) {
    ...
    try {
      ...
      if (crossValidationResult) {
        ...
        // Single response validation - focus on technical quality and content analysis
        const responseLength = responseText.length;
        const lengthScore = Math.min(100, (responseLength / 500) * 100); // Favor longer, detailed responses
        const technicalQuality = technicalValidationResult?.validationScore || 75;
        const gapPenalty = analysisResult?.gaps?.length > 0 ? Math.max(0, 80 - (analysisResult.gaps.length * 10)) : 80;
        
        confidenceScore = Math.round(
          (technicalQuality * 0.45) +     // Technical validation is primary
          (lengthScore * 0.25) +           // Response completeness
          (gapPenalty * 0.20) +            // Known gaps reduce confidence
          (analysisScore * 0.10)           // Content analysis
        );
      }
