      ...
      const animate = () => {
        ...
        nodes.forEach((node, i) => {
          ...
          node.connections?.forEach((targetIdx) => {
            ...
            ctx.lineTo(tx, ty);
            ctx.strokeStyle =
              isHighlighted || targetHighlighted
...
const WorkSurfaceLayer = memo(
  ({ children, viewMode = 'micro', dimmed = false, focusTarget = null }) => {
      ...
      <div className="absolute inset-0 z-50 pointer-events-none">
        ...
        <div className="absolute top-0 left-0 right-0 h-12 flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs font-mono uppercase ${autonomyEnabled ? 'text-amber-400' : 'text-cyan-400'}`}
            >
...
export const LiquidShell = memo(
  ({ children, showEdge = true, className = '', initialMood = 'calm', dependencies = [] }) => {
    // Deep Canvas state
    ...
    const toggleAutonomy = useCallback(() => {
      setAutonomyEnabled((prev) => {
        ...
        window.dispatchEvent(
          new CustomEvent('tooloo:autonomy', {
    ...
    const getTaskOptions = useCallback(() => {
      return autonomyEnabled ? { autoApprove: true, allowSelfModification: true } : {};
    }, [autonomyEnabled]);
...
export const LiquidTransition = memo(({ children, viewKey, className = '' }) => {
  ...
  useEffect(() => {
    if (viewKey !== prevKeyRef.current && containerRef.current) {
      ...
      el.style.opacity = '0';
      el.style.transform =
        viewMode === 'macro' ? 'scale(0.95) translateY(-10px)' : 'scale(1.02) translateY(10px)';
