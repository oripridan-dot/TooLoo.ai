...
const SegmentItem = ({ segment, index, isActive, onClick }) => {
  ...
  const title =
    segment.title || segment.summary || segment.content?.slice(0, 50) || `Segment ${index + 1}`;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderRadius: '8px',
        cursor: 'pointer',
        background: isActive ? 'var(--accent-primary-dim, rgba(155,89,182,0.15))' : 'transparent',
        borderLeft: isActive ? '3px solid var(--accent-primary, #9B59B6)' : '3px solid transparent',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p
          style={{
            ...
            color: isActive ? 'var(--text-primary, #fff)' : 'var(--text-secondary, #888)',
            overflow: 'hidden',
...
const DAGView = ({ dag, onNodeClick }) => {
  ...
  return (
    ...
    >
      ...
      <svg width="100%" height={Math.max(80, nodes.length * 30)} style={{ display: 'block' }}>
        {/* Draw edges */}
        {edges.map((edge, i) => {
        ...
        {nodes.map((node, i) => (
          ...
          >
            <circle
              ...
              strokeWidth={1}
            />
            <text x={35} y={i * 30 + 18} fontSize="11" fill="var(--text-secondary, #888)">
      ...
      {isOpen && (
        ...
        >
          ...
          {skills.executionStatus !== 'idle' && (
            <ExecutionStatus status={skills.executionStatus} currentTask={skills.currentTask} />
          )}
